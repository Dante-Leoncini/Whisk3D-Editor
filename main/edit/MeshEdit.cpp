...
#include "objects/Armature.h"   // rename de vertex group: arrastrar el hueso homonimo del rig
#include "animation/Armature2DAnimation.h" // rename de UV group: arrastrar el hueso 2D homonimo
#include "W3dNombres.h"         // LA regla de nombres unicos (compartida)
#include "W3dAviso.h"           // avisos con nombres SIN desbordar (declara Notificar; ver W3dAviso.h)
#include "w3dGraphics.h"
#include "WhiskUI/theme/colores.h"
#include "render/OpcionesRender.h"
#include "io/JsonW3d.h"         // parser del sidecar <modelo>.pvs.json (modificador Culling/PVS)
#include "w3dFilesystem.h"      // ReadFileBytes: el sidecar puede ser una ENTRADA del .w3d v4
#include <fstream>
#include <iostream>
#include <math.h>
#include <set>
...
void MergeVertsEdit(Mesh* m, int modo, float dist, const Vector3& cursorLocal) {
    ...
    //    El guard re-ancla sharp/seam (claves por POSICION) al cerrar el scope: sin el, soldar
    //    vertices tiraba las marcas de TODA la malla, no solo las de las aristas que colapsan.
    { W3dMoverVerts mv(m);
    ...
    //    sin importar el orden) queda UNA sola (un "merge at center" fallido dejaba caras encimadas).
    m->PoblarCapas();
    std::vector<MeshFace> nf3d; std::vector<int> survCorner; int Lold = 0;
...
// DELETE > EDGE LOOPS (inverso del loop cut): disuelve el edge loop seleccionado uniendo los quads de
...
void Mesh::ReconstruirEditSelPorPos(const std::vector<Vector3>& posSel) {
    ...
    // que el 2do extrude de un vertice viera 2 verts (un borde) y saliera un quad en vez de una linea.
    for (int gv=0; gv<vertexSize; gv++)
...
// ============================================================================
//  RE-ANCLAJE de sharp/seam EN EL EXTRUDE
//
//  El extrude NO es un move puro (no le sirve el guard W3dMoverVerts, que exige
//  la MISMA numeracion de vertices antes y despues): crece los arrays y ADEMAS
//  desplaza por 'eps' las posiciones de la parte INTERIOR de la region. Con la
//  malla entera seleccionada eso mueve TODOS los vertices, ninguna clave (que
//  es la POSICION cruda) matchea y la poda de CalcularBordes se llevaba puestas
//  las 12 sharp + 12 seam del cubo aunque la topologia quedara IDENTICA.
//
//  Aca se re-ancla A MANO, y SOLO EL TRAMO MOVIDO:
//    'movidas' = las posiciones VIEJAS que DEJAN DE EXISTIR (los verts
//                interiores, que el extrude desplazo). Una marca que no toca
//                ninguna de esas queda EXACTAMENTE COMO ESTABA: su arista
//                sigue donde estaba (extruir 1 cara no mueve nada, y ahi el
//                comportamiento de siempre no cambia en nada).
//    'region'  = pos VIEJA -> pos en la TAPA, para TODA la region extruida:
//                los interiores (que se movieron) y los del CONTORNO (que no
//                se mueven, pero cuya continuacion en la superficie extruida es
//                su vertice DUPLICADO). Asi una marca interior->contorno queda
//                sobre la arista de la TAPA, que es la que de verdad existe.
//  Lo que quede sin arista lo tira igual la poda, que corre a continuacion.
// ============================================================================
static void ExtrudeReanclarMarcas(std::set<std::string>& marcas,
                                  const std::map<std::string, std::string>& region,
                                  const std::set<std::string>& movidas) {
    if (marcas.empty() || movidas.empty()) return;   // sin tramo movido no hay nada que seguir
    ...
    for (std::set<std::string>::const_iterator it = marcas.begin(); it != marcas.end(); ++it) {
        ...
        if (!movidas.count(a) && !movidas.count(b)) { nuevo.insert(*it); continue; } // el extrude no la movio
        std::map<std::string, std::string>::const_iterator ia = region.find(a);
        std::map<std::string, std::string>::const_iterator ib = region.find(b);
        if (ia != region.end()) a = ia->second;
        if (ib != region.end()) b = ib->second;
        if (a == b) continue;                                         // los dos extremos encimados: se colapso
...
bool Mesh::ExtruirEdit(Vector3& outDirLocal, bool& outConstrain) {
    ...
    // normal (aunque toquen caras): su extrude es LIBRE, desde la VISTA. nrm queda solo para
    ...
    // ANIM UV DE LA TAPA (consistente con duplicar, ver DuplicarSeleccionEdit): cada vert
    // nuevo del extrude deja una RECETA soloUV con su vert de ORIGEN -> hereda el canal UV
    // animado del origen en todos los keyframes (posiciones/normales ESTATICAS, regla previa).
    // El dueno pidio "cara o vertice" duplicados; extrude se suma por consistencia: el mapeo
    // de origen ya existe (newOf) y el vert nuevo nace con el UV vivo del origen igual.
    ...
    for (std::set<int>::iterator it=capReps.begin(); it!=capReps.end(); ++it) { int r=*it;
        ...
        esCap[wi]=1; newOf[r]=wi;
        { VtxRecipe rc(r, r, 0.0f); rc.soloUV = true; vtxAnimRecipes.push_back(rc); }
        wi++;
    }
    // ---- RE-ANCLAR sharp/seam del TRAMO MOVIDO (ver ExtrudeReanclarMarcas, arriba) ----
    // ULTIMO MOMENTO en que se pueden leer las posiciones VIEJAS (vertex[]) y las NUEVAS
    // (nVtx) a la vez. Entre el desplazamiento de los interiores y este punto no se
    // recalculan los bordes, asi que la poda no llega a ver el estado intermedio.
    if (!sharpEdges.empty() || !seamEdges.empty()) {
        std::map<std::string, std::string> region;   // pos VIEJA de la region -> su pos en la TAPA
        std::set<std::string> movidas;               // pos VIEJAS que dejan de existir (interiores)
        for (int gi = 0; gi < nV; gi++) {
            if (!esCap[gi]) continue;                // en [0,nV) esCap marca justo a los INTERIORES movidos
            std::string kv((const char*)&vertex[gi*3], 12);
            region.insert(std::make_pair(kv, std::string((const char*)&nVtx[gi*3], 12)));
            movidas.insert(kv);
        }
        for (std::map<int,int>::iterator it = newOf.begin(); it != newOf.end(); ++it) {
            if (it->first < 0 || it->first >= nV) continue;
            region.insert(std::make_pair(std::string((const char*)&vertex[it->first*3], 12),
                                         std::string((const char*)&nVtx[it->second*3], 12)));
        }
        ExtrudeReanclarMarcas(sharpEdges, region, movidas);
        ExtrudeReanclarMarcas(seamEdges,  region, movidas);
    }
...
// no solo edgeSel: en FACE mode tomaba el edgeSel stale -> marcaba TODOS los bordes.
...
//
// INVARIANTE DE POSE, LA PUERTA QUE *CREA* LAS CLAVES: la clave se ancla contra
// Mesh::PosicionesReposo(), no contra vertex[]. Con la malla POSADA por una vertex anim
// vertex[] tiene la pose del cuadro, y anclar ahi hacia nacer la clave en el espacio de la
// POSE: el gate de la poda la protegia mientras siguiera posada, pero al volver a reposo esa
// clave no correspondia a ninguna arista y la poda -con razon: ya no esta posada- se llevaba
// TODO. Anclar contra el KEYFRAME BASE (y no prohibir marcar posado con un cartel) es lo que
// el usuario espera: marca mirando la pose que le resulta comoda y la marca queda en el
// modelo, que es donde vive. Es EL MISMO invariante que respetan la poda
// (Mesh::PodarMarcasSinArista) y el escritor del .w3dm: las tres puertas del mismo dato.
static bool MarcarUnaArista(Mesh* m, EditMesh* e, int eg, std::set<std::string>& edges, bool poner) {
    ...
    const GLfloat* P = m->PosicionesReposo();   // = vertex[] salvo POSADA por una vertex anim
    if (!P) return false;
    std::string key = Mesh::SharpEdgeKey(&P[gva*3], &P[gvb*3]);
...
void Mesh::EscribirUVProyeccion(const std::vector<float>& uvPorCorner) {
    ...
    // la seleccion ("al cambiar el UV no me cambies la seleccion").
    EnsureEdit();
    if (edit) {
...
// llena selVert[gi] (por RENDER-vert, tamano vertexSize) = 1 si el render-vert gi esta seleccionado
// en el edit 3D segun el EditSelectMode ACTUAL. Expande por posRep: TODOS los render-verts de un
// control-point seleccionado (incluidas las COPIAS por costura UV/normal) quedan marcados. Lo usa el
// editor UV en Sync Selection para reflejar la seleccion del 3D en las UV.
void Mesh::VertsSelPorModo(std::vector<char>& selVert) {
    selVert.assign(vertexSize > 0 ? (size_t)vertexSize : 0, 0);
    EnsureEdit();
    if (!edit || !vertex || vertexSize <= 0) return;
    EditMesh* e = edit;
    const int nV = vertexSize;
    const bool hayRep = ((int)posRep.size() == nV);
    // 1) selRep[rep] = representante seleccionado, segun el modo (vertice/borde/cara)
    std::vector<char> selRep(nV, 0);
    if (EditSelectMode == SelVertex) {
        for (size_t k = 0; k < e->editVerts.size(); k++)
            if (k < e->vertSel.size() && e->vertSel[k] && e->editVerts[k] >= 0 && e->editVerts[k] < nV)
    ...
    // 2) expandir a TODOS los render-verts via posRep (las copias por costura tambien quedan)
    for (int gi = 0; gi < nV; gi++) {
        int rep = (hayRep ? posRep[gi] : gi);
        if (rep >= 0 && rep < nV && selRep[rep]) selVert[gi] = 1;
    }
}

void Mesh::CarasSelPorModo(std::vector<char>& sel3d) {
    sel3d.assign(faces3d.size(), 0);
    std::vector<char> selVert; VertsSelPorModo(selVert);   // seleccion 3D por render-vert
    if ((int)selVert.size() != vertexSize) return;
    const int nV = vertexSize;
    // una cara cuenta si TODOS sus corners estan seleccionados (como Delete>Faces)
    for (size_t f = 0; f < faces3d.size(); f++) {
        const std::vector<int>& idx = faces3d[f].idx;
        if (idx.empty()) continue;
        bool todos = true;
        for (size_t c = 0; c < idx.size(); c++) {
            int gi = idx[c];
            if (gi < 0 || gi >= nV || !selVert[gi]) { todos = false; break; }
        }
        if (todos) sel3d[f] = 1;
    }
}
...
bool Mesh::DuplicarSeleccionEdit() {
    ...
    // (caras + bordes), no solo verts sueltos. En vertice/borde ademas: BORDES sueltos (no cubiertos por
    ...
    for (int gi=0;gi<nV;gi++){ nVtx[gi*3]=vertex[gi*3];nVtx[gi*3+1]=vertex[gi*3+1];nVtx[gi*3+2]=vertex[gi*3+2];
        ...
        if(tUV){nUV[gi*2]=uv[gi*2];nUV[gi*2+1]=uv[gi*2+1];}else{nUV[gi*2]=0;nUV[gi*2+1]=0;}
        if(tCol){for(int q=0;q<4;q++)nCol[gi*4+q]=vertexColor[gi*4+q];}else{for(int q=0;q<4;q++)nCol[gi*4+q]=255;} }
    // ANIM UV DEL DUPLICADO (pedido del dueno): cada vert copiado deja una RECETA soloUV con
    // su vert de ORIGEN -> RemapVertexAnims le copia el canal UV del origen en TODOS los
    // keyframes de todas las vertex anims (la capa VertexFrame::uvs; los frames sin capa uv
    // se respetan). Las POSICIONES del duplicado siguen ESTATICAS (regla previa intacta:
    // la receta soloUV NO toca posiciones/normales). Las recetas van en el MISMO orden que
    // los wi (indice de receta = wi - nV, que es lo que espera el remap).
    vtxAnimRecipes.clear(); vtxAnimRecipeBase = nV;
    std::map<int,int> newOf; int wi=nV;
    ...
    // y el siguiente GenerarRender la descarta -> "duplico un vertice solito y no lo puedo extruir").
    for (std::set<int>::iterator it=dupReps.begin(); it!=dupReps.end(); ++it){ int r=*it;
    ...
    if (huboTopo) {
        ...
        GenerarRender();  // (adentro: RemapVertexAnims consume las recetas soloUV de arriba)
        ...
        // sin GenerarRender no hay remap automatico: remapear ACA las vertex anims con el
        // mapeo IDENTIDAD (los indices no se re-numeran). Los verts copiados (>= nV) caen
        // en las recetas soloUV de arriba (UV del origen por keyframe, posiciones estaticas).
        // De paso arregla un bug previo: an->vcount quedaba viejo y EvalVertexAnim cortaba
        // la anim ENTERA al duplicar un vert suelto.
        if (!animations.empty()) {
            std::vector<int> n2o(nuevoN);
            for (int gi = 0; gi < nuevoN; gi++) n2o[gi] = gi;
            RemapVertexAnims(this, &n2o[0], nuevoN, nV);
        }
        vtxAnimRecipes.clear(); vtxAnimRecipeBase = 0; // consumidas (igual que en GenerarRender)
        ReconstruirEditSel(esCopia);  // deja seleccionada la COPIA
    }
    return true;
}

// RIP (V): SEPARA la malla a lo largo de la seleccion (el "corte"). Flood-fill de las caras en
...
bool Mesh::CrearCaraEdit() {
    ...
    if (reps.size() == 2) {
        ...
        // no crear otra. Antes, apretar "F" de nuevo sobre los mismos verts apilaba caras iguales encimadas
...
static bool SubdivBordeSuelto(Mesh* m, int startEditEdge, int numCuts, float factor, std::vector<Vector3>* outSel){
    ...
    m->vtxAnimRecipes.clear(); m->vtxAnimRecipeBase = nV; // receta: cada cut = lerp(ga,gb,s) -> sigue la anim
    for (int j=0;j<numCuts;j++){ float s=(float)(j+1+factor)/(float)(numCuts+1); if(s<0)s=0; if(s>1)s=1;
        ...
        m->vtxAnimRecipes.push_back(Mesh::VtxRecipe(ga, gb, s));
    }
...
//
// LIMITACION CONOCIDA (a proposito, no es un bug a "arreglar" aca): los verts NUEVOS del corte
// NO heredan peso en los UV GROUPS (pesos por corner) -> el loop nuevo queda en peso 0 en vez de
// interpolar entre los dos bordes que corta. Es CONSISTENTE con los VERTEX GROUPS (por
// control-point), que hacen exactamente lo mismo: las ops de topologia (loop cut,
// subdividir, extrude) no interpolan pesos; el peso se re-pinta. Interpolar por corner sin
...
bool Mesh::LoopCutEdit(int startEditEdge, int numCuts, float factor) {
    ...
    // RECETA para la vertex anim: cada vert de corte = interpolacion de 2 verts viejos.
    // Se llena EN PARALELO a vp (index del vert nuevo - nV = index en vtxAnimRecipes) y la
    // consume RemapVertexAnims (via GenerarRender) para que el corte SIGA la animacion.
    vtxAnimRecipes.clear(); vtxAnimRecipeBase = nV;
    ...
    for (int i=0;i<L;i++){
        ...
        for (int j=0;j<numCuts;j++){
            ...
            vtxAnimRecipes.push_back(VtxRecipe(gA0, gB0, s));  // vert ni = lerp(gA0,gB0,s) -> receta
            int no=(int)(vp.size()/3);
            for (int q=0;q<3;q++) vp.push_back(vertex[gA1*3+q]*(1-s)+vertex[gB1*3+q]*s);
            cvOut[j]=no; cutVerts.push_back(no);
            vtxAnimRecipes.push_back(VtxRecipe(gA1, gB1, s));  // vert no = lerp(gA1,gB1,s) -> receta
        }
...
// verts que YA se pegaron al plano en ESTE transform (clave = editK<<8 | (mi<<2) | eje). Una vez pegado, queda
...
void Mesh::ClipMirrorVerts(const std::vector<int>& editKs, const std::vector<Vector3>& startLocal) {
    ...
    for (size_t mi=0; mi<modificadores.size(); mi++){
        ...
        if (mod->target){ // plano del target llevado al espacio LOCAL del objeto
            // BASE las dos: el plano del Mirror decide DONDE QUEDAN LOS VERTICES. Si saliera de
            // la transform efectiva, un billboard en el objeto o en el target moveria el plano al
            // orbitar y la geometria generada cambiaria con la camara (ver Objects.h).
            Matrix4 Wo, Wt; GetWorldMatrixBase(Wo); mod->target->GetWorldMatrixBase(Wt);
...
void Mesh::RenderEditOverlay() {
    EnsureEdit();
    if (edit) {
        // AUTO-SYNC del cage con el render: si algo movio vertex[] POR FUERA de la
        // edicion, el overlay (pos[]/lineas/caras) quedo en la pose vieja. Caso tipico:
        // una VERTEX ANIM parada en un frame del timeline (EvalVertexAnim deforma
        // vertex[] y bumpea skinGeomVersion). Re-derivamos el cage desde vertex[] para
        // que COINCIDA con el render deformado (y con el picking, que usa el mismo pos[]).
        // Solo dispara cuando cambia skinGeomVersion: editar verts empuja pos[]->vertex
        // via EmpujarPosiciones SIN bumpear la version, asi que la edicion en curso jamas
        // gatilla un re-sync que la pise. Las ops de topologia hacen edit=NULL (rebuild
        // via Construir), no pasan por este if.
        if (edit->syncedGeomVersion != skinGeomVersion) edit->SincronizarPos();
        edit->Render();
    }
}
...
void Mesh::AnexarMallaTransformada(Mesh* otra, const Matrix4& M) {
    ...
    // --- SHARP / SEAM de 'otra': hay que TRAERLAS, reflejadas por M ---------------------------
    //  Las de ESTA sobreviven solas (sus posiciones no se mueven). Las de 'otra' estan en SU
    //  espacio local y sus claves son los BYTES de esas posiciones: despues del join no cae
    //  ningun punto ahi y las marcas se perdian ENTERAS y en silencio (marcar costuras en B,
    //  Ctrl+J con A activo, y B llegaba sin una sola costura).
    //  El remapeo NO recalcula M*v otra vez: usa los bytes que ACABAN de escribirse en nv[],
    //  asi la clave nueva es exactamente la posicion que la malla tiene (recalcular con otra
    //  secuencia de operaciones podria diferir en el ultimo bit y dejar la marca huerfana).
    if (!otra->sharpEdges.empty() || !otra->seamEdges.empty()) {
        //  DE QUE LADO SE LEE LA CLAVE VIEJA: del REPOSO de 'otra', no de su vertex[]. Si 'otra'
        //  esta POSADA por una vertex anim, vertex[] tiene la pose y sus claves -que viven en el
        //  espacio de reposo- no matchean NINGUNA entrada del mapa: el join se comia todas las
        //  marcas de 'otra' en silencio. PosicionesReposo devuelve vertex[] cuando no esta posada
        //  (el caso normal), asi que es la misma cuenta de siempre.
        const GLfloat* Rotra = otra->PosicionesReposo();
        if (!Rotra) Rotra = otra->vertex;
        std::map<std::string, std::string> remap;    // posicion de REPOSO de 'otra' -> posicion en ESTA
        for (int i = 0; i < add; i++)
            remap.insert(std::make_pair(std::string((const char*)&Rotra[i*3], 12),
                                        std::string((const char*)&nv[(base+i)*3], 12)));
        for (int pasada = 0; pasada < 2; pasada++) {
            const std::set<std::string>& src = pasada ? otra->seamEdges : otra->sharpEdges;
            std::set<std::string>&       dst = pasada ? seamEdges       : sharpEdges;
            for (std::set<std::string>::const_iterator it = src.begin(); it != src.end(); ++it) {
                if (it->size() != 24) continue;
                std::map<std::string, std::string>::const_iterator ia = remap.find(it->substr(0, 12));
                std::map<std::string, std::string>::const_iterator ib = remap.find(it->substr(12, 12));
                if (ia == remap.end() || ib == remap.end()) continue;   // extremo que no es de esta malla
                const std::string& a = ia->second; const std::string& b = ib->second;
                if (a == b) continue;                                   // la arista se colapso
                dst.insert((a < b) ? (a + b) : (b + a));                // el borde no tiene direccion
            }
        }
    }
    ...
    //     El NOMBRE se uniquifica: dos mallas con una parte "Mesh" cada una dejaban DOS partes "Mesh"
    //     en la unida (el espacio de nombres de las partes es la malla). ---
    for (size_t g=0; g<otra->materialsGroup.size(); g++){
        MaterialGroup mg = otra->materialsGroup[g];
        mg.name = NombreLibreMeshPart(mg.name, -1);
        materialsGroup.push_back(mg);
    }
    ...
    // --- verts sueltos: mismo remapeo que looseEdges (antes se perdian en el join) ---
    for (size_t i=0;i<otra->looseVerts.size();i++)
        looseVerts.push_back(otra->looseVerts[i]+base);
    ...
    // --- UV GROUPS (pesos por CORNER del editor UV): se mergean POR NOMBRE igual que los vertex
    // groups, pero sus indices son RENDER-VERTS -> se desplazan con 'base' (el mismo offset de
    // looseEdges/looseVerts), NO con cpOffset. GenerarRender del caller los remapea despues. ---
    for (size_t g=0; g<otra->uvGroups.size(); g++){ UVGroup* bg = otra->uvGroups[g]; if (!bg) continue;
        UVGroup* ag = NULL; for (size_t k=0;k<uvGroups.size();k++) if (uvGroups[k] && uvGroups[k]->nombre==bg->nombre){ ag=uvGroups[k]; break; }
        if (!ag){ ag = new UVGroup(bg->nombre); uvGroups.push_back(ag); }
        for (size_t v=0; v<bg->verts.size() && v<bg->pesos.size(); v++){ ag->verts.push_back(bg->verts[v] + base); ag->pesos.push_back(bg->pesos[v]); }
    }
    if (uvGrupoActivo < 0 && !uvGroups.empty()) uvGrupoActivo = 0;
}
...
void Mesh::AplicarMatriz(const Matrix4& B) {
    if (!vertex || vertexSize <= 0) return;
    {
        ...
        // marcar las costuras y despues Apply Scale antes de desplegar UV tiraba TODAS
        // las marcas (se seguian dibujando, pero el .w3dm salia sin SHARP ni SEAM). El
        // guard re-ancla al cerrar el scope; cierra ANTES del GenerarRender de abajo
        // porque ese re-numera los verts.
        W3dMoverVerts mv(this);
...
//  generacion/procesamiento vive aca todavia (por ahora, solo el stack y su orden).
// ============================================================================
// CONTADOR de identidad estable de los modificadores (ver W3dModSerial en edit/Modifier.h).
// Mismo criterio que Object::serial: monotono, sin reuso, arranca en 1 para que 0 signifique
// "ninguno". Reciclarlo seria exactamente el bug que evita.
static unsigned int W3dModSerialProx = 1;
W3dModSerial::W3dModSerial() : v(W3dModSerialProx++) {}
W3dModSerial::W3dModSerial(const W3dModSerial&) : v(W3dModSerialProx++) {}

// nombre UNICO del modificador dentro de la malla (dos Mirror daban dos "Mirror")
static bool ModNombreExiste(const std::string& n, void* ctx) {
    Mesh* m = (Mesh*)ctx;
    for (size_t i = 0; i < m->modificadores.size(); i++)
        if (m->modificadores[i] && m->modificadores[i]->nombre == n) return true;
    return false;
}
void Mesh::AgregarModificador(int tipo) {
    const std::string nom = W3dNombreUnico(NombreTipoModificador(tipo), "Modifier", ModNombreExiste, this);
    modificadores.push_back(new Modifier(tipo, nom));
    modificadorActivo = (int)modificadores.size() - 1; // el nuevo queda seleccionado
}
// OJO: estas tres NO dejan paso de undo. La puerta del USUARIO es UndoModAgregar /
// UndoModQuitar / UndoModMover (undo/Undo.h), que capturan el stack y despues llaman aca.
// Estas quedan para los caminos que NO quieren un paso: la carga del .w3d y el destructor.
// SACA el activo del stack y lo DEVUELVE sin liberarlo: el que llama pasa a ser su dueno.
// Existe para que el paso de undo pueda ADOPTARLO (ModStackUndo): si el Remove lo liberara, el
// Ctrl+Z no tendria que devolver, y ademas la direccion quedaria libre para que la reciclara
// otro modificador -con el DeleteUndo todavia guardandose ese puntero-. NULL = no habia activo.
Modifier* Mesh::SacarModificadorActivo() {
    if (modificadorActivo < 0 || modificadorActivo >= (int)modificadores.size()) return NULL;
    Modifier* m = modificadores[modificadorActivo];
...
void Mesh::QuitarModificadorActivo() {
    delete SacarModificadorActivo();   // sin dueno que lo adopte: se libera (ver arriba)
}
...
static bool AristaEnPlanoMirror(Mesh* m, int ra, int rb){
    ...
    for (size_t mi=0; mi<m->modificadores.size(); mi++){
        ...
        if (mod->tipo != ModifierType::Mirror || !mod->clipping) continue;
        Vector3 c(0,0,0), ax0(1,0,0), ax1(0,1,0), ax2(0,0,1);
        if (mod->target){ // plano del target -> espacio LOCAL del objeto (igual que ClipMirrorVerts: BASE)
            Matrix4 Wo, Wt; m->GetWorldMatrixBase(Wo); mod->target->GetWorldMatrixBase(Wt);
            Matrix4 iWo; InvAffineMod(Wo, iWo); Matrix4 M = iWo * Wt;
            c   = Vector3(M.m[12],M.m[13],M.m[14]);
            ax0 = NormMod(Vector3(M.m[0],M.m[1],M.m[2]));
            ax1 = NormMod(Vector3(M.m[4],M.m[5],M.m[6]));
...
static bool Mesh_AplicarStack(Mesh* m, bool render, PolyMesh& W, bool& outSmooth) {
    ...
    for (size_t i=0; i<m->modificadores.size(); i++){ Modifier* mod = m->modificadores[i];
        ...
        if (mod->tipo == ModifierType::SubdivisionSurface){
            ...
            if (mod->target){ Matrix4 Wo, Wt; m->GetWorldMatrixBase(Wo); mod->target->GetWorldMatrixBase(Wt);
                Matrix4 iWo; InvAffineMod(Wo, iWo); Matrix4 M = iWo * Wt;
                c   = Vector3(M.m[12],M.m[13],M.m[14]);
...
// ============================================================================
//  PVS — modificador "Culling" POR TRIANGULO (metodo Triangulos, estilo PS1).
//
//  Los datos vienen PRECALCULADOS del juego original en un sidecar junto al .obj
//  de ORIGEN de la malla (Mesh::origen, el que importo ImportWOBJ):
//      <modelo>.pvs.json = { "sectores": M, "tris": [[t0, t1, ...], ...] }
//  con cada t = indice de TRIANGULO 0-based en el ORDEN DE CARAS del OBJ (el
//  contrato del exportador del juego; las caras del OBJ son triangulos, asi que
//  el triangulo t vive en faces[3t..3t+2]; con varios mesh parts se asume que
//  faces[] conserva el orden del archivo, que es lo que garantiza el pipeline
//  del proyecto: 1 usemtl = 1 mesh part).
//
//  El modificador NO genera malla (la editable queda INTACTA): precalcula UN
//  index buffer por sector (aca, W3dPVSSincronizar) y lo instala como override
//  en el core (Mesh::pvsFaces/pvsGroups). En runtime cambiar de sector es un
//  SWAP de indices (el fast-path VBO re-sube SOLO el IBO al ver pvsVersion
//  distinto): NUNCA trabajo por frame. El lua del juego llama setSector(obj, s)
//  al cambiar el nodo/camara del riel; s=0 o fuera de rango = malla completa.
// ============================================================================
static Modifier* PVSModDe(Mesh* m) {
    if (!m) return NULL;
    for (size_t i = 0; i < m->modificadores.size(); i++)
        if (m->modificadores[i] && m->modificadores[i]->tipo == ModifierType::CullingTri)
            return m->modificadores[i];
    return NULL;
}

// ruta de un sidecar derivado: <origen sin extension> + sufijo ("" = sin .obj de origen)
static std::string PVSRutaDerivada(Mesh* m, const char* sufijo) {
    if (!m || m->origen.empty()) return std::string();
    std::string ruta = m->origen;
    size_t punto = ruta.find_last_of('.');
    size_t barra = ruta.find_last_of("/\\");
    if (punto != std::string::npos && (barra == std::string::npos || punto > barra))
        ruta = ruta.substr(0, punto);
    return ruta + sufijo;
}
static std::string PVSRutaSidecar(Mesh* m) { return PVSRutaDerivada(m, ".pvs.json"); }

// ---------------------------------------------------------------------------
//  metodo "celdas": carga el `.w3dvis` (VisSet, formato/w3dvis.md) del modificador.
//  Mismos TRES candidatos que el sidecar JSON: nombre guardado (entrada real del
//  contenedor v4), <origen sin extension>.w3dvis, extra/<base>.w3dvis. Tolerante:
//  sin archivo = sin celdas = malla completa (y el panel lo muestra).
//  Si el archivo trae hash de topologia y NO coincide con las caras actuales, se
//  AVISA (warning, nunca error duro): la malla cambio despues del bake.
// ---------------------------------------------------------------------------
static bool W3dVisCargarDato(Mesh* m, Modifier* mod) {
    if (!mod) return false;
    mod->visSet.Limpiar();
    mod->visLista.clear();
    mod->pvsAplicado = -1;
    std::string derivada = PVSRutaDerivada(m, ".w3dvis");
    std::vector<std::string> cands;
    if (!mod->visArchivo.empty()) cands.push_back(mod->visArchivo);
    if (!derivada.empty()) {
        cands.push_back(derivada);
        size_t b = derivada.find_last_of("/\\");
        cands.push_back("extra/" + (b == std::string::npos ? derivada : derivada.substr(b + 1)));
    }
    std::vector<unsigned char> bytes;
    std::string ruta;
    for (size_t i = 0; i < cands.size(); i++) {
        if (w3dFileSystem::ReadFileBytes(cands[i], bytes) && !bytes.empty()) { ruta = cands[i]; break; }
        bytes.clear();
    }
    if (ruta.empty()) return false;   // sin dato: no es error (malla completa)
    std::string verr;
    if (!mod->visSet.CargarW3dvis(&bytes[0], bytes.size(), &verr)) {
        std::cerr << "[Vis] .w3dvis invalido (" << verr << "): " << ruta << std::endl;
        return false;
    }
    if (mod->visSet.hashTopo != 0 && m->faces && m->facesSize >= 3) {
        unsigned h = (sizeof(MeshIndex) == 2)
            ? VisSet::HashTopologia16(m->facesSize / 3, (const unsigned short*)m->faces, m->facesSize)
            : VisSet::HashTopologia(m->facesSize / 3, (const unsigned int*)m->faces, m->facesSize);
        if (h != mod->visSet.hashTopo)
            std::cerr << "[Vis] AVISO: la topologia de '" << m->name
                      << "' no coincide con el bake de " << ruta
                      << " (la malla cambio despues de hornear)" << std::endl;
    }
    return mod->visSet.Valido();
}

// ---------------------------------------------------------------------------
//  arma el override Mesh::pvsFaces/pvsGroups desde una lista de TRIANGULOS.
//  El ORDEN de la lista se conserva DENTRO de cada mesh part (los grupos siguen
//  el orden de materialsGroup, que con z-buffer es gratis entre opacos; las
//  listas ordenadas lejos->cerca conservan el orden del pintor por grupo).
//  Indices fuera de la malla se IGNORAN (dato de otra version: mejor dibujar de
//  menos que pinchar). Comun a los dos metodos (sectores JSON y celdas VisSet).
// ---------------------------------------------------------------------------
static void PVSArmarOverride(Mesh* m, const unsigned* tris, size_t nLista, bool ordenado) {
    extern bool g_redraw;
    const int nTrisMalla = m->facesSize / 3;
    const size_t nG = m->materialsGroup.size();
    std::vector<MeshIndex> idx; idx.reserve(nLista * 3);
    std::vector<MaterialGroup> grupos = m->materialsGroup;
    m->pvsRuns.clear();
    // (E3) con lista ORDENADA (lejos->cerca): los mesh parts TRANSPARENTES no se
    // agrupan; sus tris salen al final EN EL ORDEN de la lista (runs por material).
    std::vector<char> esTrans(nG, 0);
    bool hayTrans = false;
    if (ordenado) for (size_t g = 0; g < nG; g++) {
        Material* mt = m->materialsGroup[g].material;
        if (mt && mt->transparent) { esTrans[g] = 1; hayTrans = true; }
    }
    // 1) OPACOS (y todo, si no hay orden): agrupados por mesh part, como siempre.
    //    El orden de la lista se conserva DENTRO de cada grupo.
    for (size_t g = 0; g < nG; g++) {
        int ini = (int)idx.size();
        if (!(ordenado && esTrans[g])) {
            int a = m->materialsGroup[g].startDrawn;
            int b = a + m->materialsGroup[g].indicesDrawnCount;
            for (size_t k = 0; k < nLista; k++) {
                unsigned t = tris[k];
                if (t >= (unsigned)nTrisMalla) continue;
                int base = (int)t * 3;
                if (base < a || base >= b) continue;   // el triangulo no es de este mesh part
                idx.push_back(m->faces[base]); idx.push_back(m->faces[base + 1]); idx.push_back(m->faces[base + 2]);
            }
        }
        grupos[g].startDrawn = ini;
        grupos[g].indicesDrawnCount = (int)idx.size() - ini;   // transparente ordenado -> 0 (vive en los runs)
    }
    // 2) TRANSPARENTES en el ORDEN EXACTO de la lista (el orden del pintor del dato):
    //    runs contiguos que se cortan al cambiar de mesh part. El render los dibuja
    //    despues de los opacos, run por run (Mesh::RenderObject, pvsRuns).
    if (ordenado && hayTrans) {
        int runGrupo = -1; size_t runStart = 0;
        for (size_t k = 0; k < nLista; k++) {
            unsigned t = tris[k];
            if (t >= (unsigned)nTrisMalla) continue;
            int base = (int)t * 3;
            ...
            for (size_t gg = 0; gg < nG; gg++) {
                int a = m->materialsGroup[gg].startDrawn;
                int b = a + m->materialsGroup[gg].indicesDrawnCount;
                if (base >= a && base < b) { g = (int)gg; break; }
            }
            if (g < 0 || !esTrans[g]) continue;
            if (g != runGrupo) {
                if (runGrupo >= 0 && idx.size() > runStart) {
                    Mesh::PvsRun run; run.grupo = runGrupo; run.start = (int)runStart;
                    run.count = (int)(idx.size() - runStart);
                    m->pvsRuns.push_back(run);
                }
                runGrupo = g; runStart = idx.size();
            }
            idx.push_back(m->faces[base]); idx.push_back(m->faces[base + 1]); idx.push_back(m->faces[base + 2]);
        }
        if (runGrupo >= 0 && idx.size() > runStart) {
    ...
    delete[] m->pvsFaces;
    m->pvsFacesSize = (int)idx.size();
    m->pvsFaces = new MeshIndex[idx.empty() ? 1 : idx.size()];   // size 0 valido: celda vacia
    for (size_t k = 0; k < idx.size(); k++) m->pvsFaces[k] = idx[k];
    m->pvsGroups.swap(grupos);
    m->pvsVersion++;                    // CONTRATO con el render: la version sube en cada cambio
    g_redraw = true;
}
...
//
// TRES CANDIDATOS, en orden (ver Modifier::pvsArchivo):
//   1. el NOMBRE GUARDADO en el .w3d, que es la entrada real del contenedor v4;
//   2. <origen sin extension>.pvs.json, que es como se derivaba siempre (y sigue
//      siendo lo correcto con el modelo desplegado en disco);
//   3. extra/<base>, la carpeta donde el contenedor clasifica los .json, por si el
//      archivo viene de un paquete guardado sin el nombre (misma tolerancia que
//      Fuente2D con el .json de glifos).
// Se lee por w3dFileSystem, no por ifstream, porque adentro de un .w3d v4 el sidecar
// es una ENTRADA del contenedor y no un archivo del disco.
bool W3dPVSCargar(Mesh* m, Modifier* mod) {
    if (!mod) return false;
    mod->pvsCargado = true;
    mod->pvsSectores.clear();
    mod->pvsAplicado = -1;   // lo armado (si habia) ya no describe estos datos
    std::string derivada = PVSRutaSidecar(m);
    std::vector<std::string> cands;
    if (!mod->pvsArchivo.empty()) cands.push_back(mod->pvsArchivo);
    if (!derivada.empty()) {
        cands.push_back(derivada);
        size_t b = derivada.find_last_of("/\\");
        cands.push_back("extra/" + (b == std::string::npos ? derivada : derivada.substr(b + 1)));
    ...
    if (ruta.empty()) return false;   // sin sidecar: no es error (malla completa)
    std::string js((const char*)&bytes[0], bytes.size());
    JParser par(js.c_str(), js.size());
    JVal* root = par.Valor();
    if (root && root->tipo == 4) {
        JVal* tris = JHijo(root, "tris", 5);
        ...
        // (un sector vacio es valido: desde ahi no se ve ningun triangulo)
        int nsec = JI(root, "sectores", (int)mod->pvsSectores.size());
        while ((int)mod->pvsSectores.size() < nsec) mod->pvsSectores.push_back(std::vector<int>());
    } else {
        std::cerr << "[PVS] sidecar invalido (no es un objeto JSON): " << ruta << std::endl;
    }
    delete root;
    return !mod->pvsSectores.empty();
}

// estado DESEADO (stack + sectorPVS) -> estado ARMADO (Mesh::pvsFaces). Idempotente y
// barata cuando no hay nada que cambiar: la llama GenerarMallaModificada (que corre en
// cada cambio de stack/geometria), setSector y el panel. NUNCA por frame de render.
void W3dPVSSincronizar(Mesh* m) {
    if (!m) return;
    extern bool g_redraw;
    Modifier* mod = PVSModDe(m);
    // sin modificador (o apagado en viewport): la malla vuelve COMPLETA
    if (!mod || !mod->mostrarViewport) {
        if (m->pvsFaces) { m->PvsLimpiar(); g_redraw = true; }
        return;
    }
    const bool celdas = (mod->metodoPVS == 1);   // 1 = "Celdas (VisSet)"; 0 = sectores JSON
    if (!mod->pvsCargado) {
        if (celdas) { W3dVisCargarDato(m, mod); mod->pvsCargado = true; }
        else W3dPVSCargar(m, mod);
    }
    int sector = mod->sectorPVS;
    int nSets = celdas ? (mod->visSet.Valido() ? mod->visSet.nCeldas : 0)
                       : (int)mod->pvsSectores.size();
    bool valido = (sector >= 1 && sector <= nSets);
    if (!valido) {   // 0 o fuera de rango = malla completa (fallback del contrato)
        if (m->pvsFaces) { m->PvsLimpiar(); g_redraw = true; }
        mod->pvsAplicado = 0; mod->pvsFacesBase = m->facesSize;
        mod->visLista.clear();
        return;
    }
    if (m->pvsFaces && mod->pvsAplicado == sector && mod->pvsFacesBase == m->facesSize
        && m->pvsGroups.size() == m->materialsGroup.size())
        return;   // ya esta armado este sector sobre esta geometria
    if (!m->faces || m->facesSize < 3) { if (m->pvsFaces) m->PvsLimpiar(); return; }

    if (celdas) {
        // la LISTA de la celda, en el orden del archivo. Cambio +-1 con lista vigente
        // (el caso del riel avanzando nodo a nodo) = UN registro delta aplicado o
        // invertido; cualquier otro salto re-decodifica desde la celda-clave.
        int desde = (mod->pvsAplicado >= 1 && !mod->visLista.empty()) ? mod->pvsAplicado - 1 : -1;
        bool ok;
        if (desde >= 0 && (sector - 1 == desde + 1 || sector - 1 == desde - 1))
            ok = mod->visSet.Delta(desde, sector - 1, mod->visLista);
        else
            ok = mod->visSet.Decodificar(sector - 1, mod->visLista);
        if (!ok) {   // dato corrupto: mejor la malla completa que un estado a medias
            std::cerr << "[Vis] decode fallo en celda " << sector << " de '" << m->name << "'" << std::endl;
            if (m->pvsFaces) { m->PvsLimpiar(); g_redraw = true; }
            mod->pvsAplicado = 0; mod->visLista.clear();
            return;
        }
        m->pvsOrdenado = mod->visSet.ordenLejosCerca;   // la lista trae el orden del pintor
        PVSArmarOverride(m, mod->visLista.empty() ? NULL : &mod->visLista[0], mod->visLista.size(),
                         mod->visSet.ordenLejosCerca);
    } else {
        // sectores del JSON legacy (sin orden): el subconjunto POR MESH PART de siempre
        m->pvsOrdenado = false;
        const std::vector<int>& tris = mod->pvsSectores[sector - 1];
        PVSArmarOverride(m, tris.empty() ? NULL : (const unsigned*)&tris[0], tris.size(), false);
    }

    // FALLBACK DE CELDA VACIA (sectorFallback, ver Modifier.h): la celda activa
    // EXISTE pero quedo sin un solo triangulo armado (lista vacia, o todos sus
    // indices fuera de esta malla). Sin declarar, vacia = nada (oclusion real).
    // Declarado, el archivo elige el paracaidas: la malla COMPLETA (-1) o la
    // lista de OTRA celda (N). El estado del stepper NO se toca: pvsAplicado y
    // visLista siguen describiendo la celda REAL, asi el proximo paso +-1 sigue
    // aplicando su delta como si el fallback no existiera.
    if (m->pvsFacesSize == 0 && mod->sectorFallback != 0) {
        int fb = mod->sectorFallback;
        bool armado = false;
        if (fb >= 1 && fb <= nSets && fb != sector) {
            if (celdas) {
                std::vector<unsigned> lista;   // local: visLista debe seguir a la celda real
                if (mod->visSet.Decodificar(fb - 1, lista) && !lista.empty()) {
                    PVSArmarOverride(m, &lista[0], lista.size(), mod->visSet.ordenLejosCerca);
                    armado = (m->pvsFacesSize > 0);
                }
            } else {
                const std::vector<int>& tris = mod->pvsSectores[fb - 1];
                if (!tris.empty()) {
                    PVSArmarOverride(m, (const unsigned*)&tris[0], tris.size(), false);
                    armado = (m->pvsFacesSize > 0);
                }
            }
        }
        if (!armado) {   // "completa" (-1), o la celda fallback tampoco tenia nada
            m->PvsLimpiar();   // sin override = la malla entera (nunca un pozo negro)
            g_redraw = true;
        }
    }
    mod->pvsAplicado = sector;
    mod->pvsFacesBase = m->facesSize;
}

// setSector(obj, s) del lua / panel: fija el sector ACTIVO del modificador Culling de la
// malla y re-arma el index buffer. s es 1-based; 0 o fuera de rango = malla completa.
// false = la malla no tiene modificador Culling (el bind lo ignora en silencio).
bool W3dPVSAplicarSector(Mesh* m, int sector) {
    Modifier* mod = PVSModDe(m);
    if (!mod) return false;
    mod->sectorPVS = sector;
    W3dPVSSincronizar(m);
    return true;
}

// wrapper para el bind lua (BindsJuego no conoce la clase Mesh): valida el tipo.
bool W3dPVSSetSectorObj(Object* o, int sector) {
    if (!o || o->getType() != ObjectType::mesh) return false;
    return W3dPVSAplicarSector((Mesh*)o, sector);
}
// instala el hook del bind setSector al cargar el EDITOR (BindsJuego se comparte con el
// runtime 2D del juego compilado, que no linkea main/edit: alla queda en no-op).
extern bool (*W3dPVSSetSectorHook)(Object*, int);
namespace { struct PVSHookInstalar { PVSHookInstalar() { W3dPVSSetSectorHook = W3dPVSSetSectorObj; } } gPVSHookInstalar; }

// ---------------------------------------------------------------------------
//  hooks de los binds de VISIBILIDAD POR CELDA (visCelda/setVisAncla/setVisCurvaT,
//  BindsJuego.cpp). Viven aca porque el editor conoce a la vez el modificador
//  (celda de una malla) y la VisZona (main/objects); el runtime sin editor los
//  deja en NULL y los binds son no-op.
// ---------------------------------------------------------------------------
#include "objects/VisZona.h"
extern int  (*W3dVisCeldaHook)(Object*);
extern bool (*W3dVisAnclaHook)(Object*, Object*);
extern bool (*W3dVisCurvaTHook)(Object*, float, const char*);
static int VisCeldaDeObj(Object* o) {
    if (!o) return -1;
    if (o->getType() == ObjectType::viszona) return ((VisZona*)o)->celdaActual;
    if (o->getType() == ObjectType::mesh) {
        Modifier* mod = PVSModDe((Mesh*)o);
        return mod ? mod->sectorPVS : -1;
    }
    return -1;
}
static bool VisAnclaDeObj(Object* zona, Object* ancla) {
    if (!zona || zona->getType() != ObjectType::viszona) return false;
    ((VisZona*)zona)->SetAncla(ancla);
    return true;
}
static bool VisCurvaTDeObj(Object* zona, float t, const char* riel) {
    if (!zona || zona->getType() != ObjectType::viszona) return false;
    ((VisZona*)zona)->SetCurvaT(t, riel);
    return true;
}
namespace { struct VisHooksInstalar { VisHooksInstalar() {
    W3dVisCeldaHook  = VisCeldaDeObj;
    W3dVisAnclaHook  = VisAnclaDeObj;
    W3dVisCurvaTHook = VisCurvaTDeObj;
} } gVisHooksInstalar; }

// la geometria o el ORDEN de faces[] cambio (ReagruparMeshParts, edicion de topologia):
// los index buffers armados apuntan a triangulos viejos -> re-armar el sector activo.
void W3dPVSInvalidar(Mesh* m) {
    Modifier* mod = PVSModDe(m);
    if (!mod) return;
    mod->pvsAplicado = -1;
    W3dPVSSincronizar(m);
...
// para el LECTOR DE TEXTO (import_w3d, nodo hijo "CullingTri { metodo: sector: }"): crea
// el modificador y lo configura. Vive aca porque esa seccion del lector no ve la clase
// Modifier (su include esta mas abajo en el archivo, para el camino v4).
// 'archivo' = el nombre REAL del sidecar tal como quedo guardado (entrada del
// contenedor v4). Vacio = derivarlo del .obj de origen, como se hacia siempre.
...
void W3dModPVSAgregar(Mesh* m, const std::string& metodo, int sector, const std::string& archivo,
    ...
    // "celdas" = VisSet/.w3dvis. "bsp" (el metodo viejo que nunca corto nada) cae al
    // MISMO id: sin `.w3dvis` que cargar dibuja la malla completa, que es exactamente
    // lo que el BSP hacia -> los .w3d viejos no cambian de comportamiento.
    mod->metodoPVS      = (metodo == "celdas" || metodo == "bsp") ? 1 : 0;
    mod->sectorPVS      = sector;
...
// activo. Devuelve la cantidad de sectores cargados (-1 = la malla no tiene el modificador).
int W3dPVSRecalcular(Mesh* m) {
    Modifier* mod = PVSModDe(m);
    if (!mod) return -1;
    mod->pvsCargado = false;   // fuerza la re-lectura (Sincronizar relee segun el metodo)
    W3dPVSSincronizar(m);
    return (mod->metodoPVS == 1) ? (mod->visSet.Valido() ? mod->visSet.nCeldas : 0)
                                 : (int)mod->pvsSectores.size();
}

// ---------------------------------------------------------------------------
//  metodo "celdas": la API con nombre propio del contrato (misma mecanica que
//  W3dPVSAplicarSector: celda 1-based, 0 = malla completa; el resultado queda en
//  Mesh::pvsFaces/pvsGroups + pvsVersion y el render lo dibuja sin saber nada).
// ---------------------------------------------------------------------------
bool W3dVisSetCelda(Mesh* m, int celda) { return W3dPVSAplicarSector(m, celda); }

// estado del metodo "celdas" para el harness/panel: nCeldas cargadas (0 = sin dato),
// celda activa y tris de la lista aplicada. -1 = la malla no tiene el modificador
// o no esta en metodo celdas.
int W3dVisInfo(Mesh* m, int* celdaActiva, int* trisLista, bool* ordenado) {
    Modifier* mod = PVSModDe(m);
    if (!mod || mod->metodoPVS != 1) return -1;
    if (celdaActiva) *celdaActiva = mod->sectorPVS;
    if (trisLista)   *trisLista = (int)mod->visLista.size();
    if (ordenado)    *ordenado = mod->visSet.ordenLejosCerca;
...
void Mesh::GenerarMallaModificada() {
    ...
    W3dPVSSincronizar(this); // PVS (modificador Culling): instala/actualiza/limpia el override de indices segun el stack.
                             // Va ANTES del early-return: al QUITAR el ultimo modificador tambien hay que limpiar.
...
static void MirrorEditableAxis(Mesh* m, const Vector3& c, const Vector3& n, bool merge, float mergeDist) {
    ...
    // SHARP/SEAM, LAS DOS MITADES. Se creyo por error que el Apply Mirror era seguro "porque
    // los verts originales no se tocan": es FALSO con merge prendido (el caso normal), que
    // SNAPEA al plano los verts de la costura. Y el guard W3dMoverVerts tal cual no sirve aca
    // porque vertexSize CRECE (nN > nV) y el re-anclaje se apaga cuando la cantidad cambia:
    // por eso se snapshotean las nV posiciones viejas y se re-ancla ANTES de crecer (abajo).
    std::vector<GLfloat> posViejas(m->vertex, m->vertex + nV*3);
    ...
    int nN=(int)(vp.size()/3);
    // --- (1) RE-ANCLAR las nV originales ANTES de crecer: con merge, el snap al plano las
    //     movio y sus claves de posicion quedarian apuntando a donde ya no hay nada.
    //     ReanclarMarcasPos exige correspondencia POR INDICE, o sea vertexSize todavia en nV:
    //     por eso se escriben las posiciones ya snapeadas encima del buffer viejo y recien
    //     despues se realoca. Sin esto, Apply Mirror con merge (el default) tiraba TODAS las
    //     marcas de la mitad original, en silencio, igual que el Apply Scale de antes.
    if (hayMarcas) {
        for (int i=0;i<nV*3;i++) m->vertex[i]=vp[i];
        m->ReanclarMarcasPos(&posViejas[0], nV);
    }
    delete[] m->vertex;      m->vertex=new GLfloat[nN*3];      for(int i=0;i<nN*3;i++) m->vertex[i]=vp[i];
    delete[] m->normals;     m->normals=new GLbyte[nN*3];      for(int i=0;i<nN*3;i++) m->normals[i]=vn[i];
    delete[] m->uv;          m->uv=new GLfloat[nN*2];          for(int i=0;i<nN*2;i++) m->uv[i]=vu[i];
    delete[] m->vertexColor; m->vertexColor=new GLubyte[nN*4]; for(int i=0;i<nN*4;i++) m->vertexColor[i]=vc[i];
    m->vertexSize=nN;
    // --- (2) REFLEJAR las marcas a la MITAD ESPEJADA. Sus verts son NUEVOS (posiciones
    //     nuevas -> claves nuevas), asi que sin esto la mitad espejada nacia sin una sola
    //     marca: no es solo un dato que se pierde al guardar, es el SHADING (un cilindro
    //     espejado quedaba con la mitad de las tapas redondeadas) y las costuras del unwrap.
    //     mir[] es justamente "quien es el espejo de cada vert original"; para los verts
    //     soldados al plano mir[i]==i y la arista espejada ES la misma (no se duplica nada).
    if (hayMarcas) {
        std::map<std::string,int> posIdx;   // posicion (12 bytes) -> vert original (0..nV-1)
        for (int i=0;i<nV;i++) posIdx.insert(std::make_pair(std::string((const char*)&m->vertex[i*3],12), i));
        for (int pasada=0; pasada<2; pasada++){
            std::set<std::string>& dst = pasada ? m->seamEdges : m->sharpEdges;
            std::vector<std::string> nuevas;
            for (std::set<std::string>::const_iterator it=dst.begin(); it!=dst.end(); ++it){
                if (it->size()!=24) continue;
                std::map<std::string,int>::const_iterator ia = posIdx.find(it->substr(0,12));
                std::map<std::string,int>::const_iterator ib = posIdx.find(it->substr(12,12));
                if (ia==posIdx.end() || ib==posIdx.end()) continue;   // marca huerfana: no la resucito
                const int ja = mir[ia->second], jb = mir[ib->second];
                if (ja==ia->second && jb==ib->second) continue;       // arista entera sobre el plano
                if (ja<0 || jb<0 || ja>=nN || jb>=nN || ja==jb) continue;
                nuevas.push_back(Mesh::SharpEdgeKey(&m->vertex[ja*3], &m->vertex[jb*3]));
            }
            for (size_t k=0;k<nuevas.size();k++) dst.insert(nuevas[k]);
        }
    }
...
void Mesh::AplicarModificadorActivo() {
    ...
    if (mod->tipo == ModifierType::CullingTri) return; // el PVS no hornea nada: la malla ya esta intacta (el panel oculta el Apply)
    if (mod->tipo == ModifierType::Armature) {
        ...
        // la posicion actual) y saca el modificador. El skinning deja de deformar (queda congelado).
        // NO toca el esqueleto ni el export (el export exporta el rig sin hornear; esto es la accion manual opuesta).
        if (!skinArmature || !vertex || vertexSize <= 0) { UndoModQuitar(this); return; } // sin geo que hornear: solo sale del stack
        ...
        // hornear la pose MUEVE los verts -> las claves de sharp/seam (por POSICION) hay que
        // re-anclarlas o la malla horneada se guarda sin ninguna marca. El guard lo hace solo.
        { W3dMoverVerts mv(this);
        ...
        UndoModQuitar(this); UndoFundirUltimos(2); // fuera del stack, EN EL MISMO paso que el snapshot de geo de arriba
        PoblarCapas();                    // asegura las capas UV/color desde el render antes de regenerar
    ...
    if (mod->tipo == ModifierType::Mirror) {
        ...
        Vector3 c(0,0,0), axX(1,0,0), axY(0,1,0), axZ(0,0,1);
        if (mod->target){ // el plano sale del TARGET (posicion + rotacion en mundo BASE, al local del objeto)
            // BASE: el Apply HORNEA los vertices espejados en la malla editable, para siempre.
            Matrix4 Wo, Wt; GetWorldMatrixBase(Wo); mod->target->GetWorldMatrixBase(Wt);
            Matrix4 iWo; InvAffineMod(Wo, iWo); Matrix4 M = iWo * Wt;
            c   = Vector3(M.m[12],M.m[13],M.m[14]);
        ...
        UndoModQuitar(this); UndoFundirUltimos(2); // fuera del stack, EN EL MISMO paso que el snapshot de geo
    ...
    // UV GROUPS: sus indices son RENDER-VERTS y la malla es OTRA (subdivision/screw horneados) ->
    // los pesos no tienen a que apuntar. Se vacian PERO el grupo QUEDA (su nombre es el binding
    // con el hueso 2D): hay que re-pintarlo en el editor UV. Los vertex groups (por control-point)
    // SI sobreviven: se re-derivan por posicion.
    for (size_t g = 0; g < uvGroups.size(); g++)
        if (uvGroups[g]) { uvGroups[g]->verts.clear(); uvGroups[g]->pesos.clear(); }
    // horneamos TODO el stack -> vaciarlo entero (materialsGroup se conserva; GenerarRender rearma los rangos).
    // Por la puerta del undo: los modificadores NO se liberan (los adopta el paso), asi el Ctrl+Z los
    // devuelve y ademas ninguna de sus direcciones queda libre para reciclarse mientras un DeleteUndo
    // todavia se guarda uno de esos punteros (ver UndoModQuitar / ModStackUndo en undo/Undo.h).
    UndoModVaciar(this); UndoFundirUltimos(2); // EN EL MISMO paso que el snapshot de geo de arriba
...
int NuevoMeshPart(Mesh* m) {
    ...
    g.name = m->NombreLibreMeshPart(g.name, -1);
...
void DuplicarUVMapActivo(Mesh* m) {
    ...
    // nombre UNICO: antes pegaba ".001" a mano y duplicar dos veces daba DOS "UVMap.001"
    UVMap* nw = new UVMap(m->NombreLibreUVMap(src->nombre, -1));
    nw->uv = src->uv;
...
void DuplicarColorLayerActivo(Mesh* m) {
    ...
    // nombre UNICO (antes ".001" a mano, sin chequear)
    ColorLayer* nw = new ColorLayer(m->NombreLibreColor(src->nombre, -1));
...
// BORRAR una capa/grupo (el boton "-" de las tarjetas) LIBERA el dueno del nombre. Los cuatro
// Borrar*Activo capturan la geo ANTES (MeshGeoUndo ya snapshotea las 4 listas de capas, ver
// Undo.h): asi el "-" es DESHACIBLE, igual que el resto de los ops. Sin esto, borrar dejaba un
// paso de rename previo apuntando a memoria liberada -> el Ctrl+Z siguiente escribia ahi
...
// el swap NO se hace aca: lo hace UndoMoverCapaMalla, que ademas deja el paso de undo y
// REMAPEA los destinos de rename ya capturados (van por indice; ver Undo.h). Igual que
// MoverVertexGroupActivo/MoverUVGroupActivo mas abajo: antes esto reordenaba a mano, sin
// undo y sin remapeo, asi que renombrar una capa y despues moverla dejaba el Ctrl+Z
// escribiendo el nombre viejo ENCIMA de la capa de al lado (corrupcion silenciosa).
...
void MoverColorLayerActivo(Mesh* m, int dir) {   // idem MoverUVMapActivo (undo + remapeo)
    if (!m) return;
    UndoMoverCapaMalla(m, W3dRenameDest::ColorLayer, m->colorActivo, m->colorActivo + dir);
}
...
// RENAME de un vertex group (tarjeta Vertex Groups). Uniquifica en la MALLA y
// arrastra el HUESO 3D homonimo del rig ligado: el binding malla<->hueso es POR
// NOMBRE (SkinearMesh), renombrar una punta sola lo rompe EN SILENCIO (el
// personaje deja de deformar). Todo en UN paso de undo. Devuelve el nombre que
// quedo (puede diferir del pedido).
std::string VertexGroupRenombrar(Mesh* m, int idx, const std::string& pedido) {
    if (!m || idx < 0 || idx >= (int)m->vertexGroups.size() || !m->vertexGroups[idx]) return std::string();
    const std::string viejo = m->vertexGroups[idx]->nombre;
    // la otra punta del binding: el hueso homonimo del armature que deforma esta malla
    ...
    // ARRASTRE: solo si hay hueso homonimo. Sin arrastre, ponerle a un grupo el nombre de un
    // hueso existente es LEGAL (asi se lo liga a mano): no hay que sufijarlo.
    bool arrastra = false;
    if (rig)
        for (size_t b = 0; b < rig->bones.size() && !arrastra; b++)
            if (rig->bones[b].name == viejo) arrastra = true;
    // el nombre tiene que quedar libre en LAS DOS PUNTAS a la vez: uniquificar solo en la malla
    // dejaba DOS huesos homonimos cuando el nombre pedido ya era de otro hueso (grupo 'G' -> 'X'
    // con huesos 'G' y 'X' daba dos 'X'). Se excluye lo que se va a mover (este grupo y los huesos
    // que se arrastran) para que renombrar al mismo valor no de ".001".
    std::vector<std::string> tomados;
    for (size_t g = 0; g < m->vertexGroups.size(); g++)
        if ((int)g != idx && m->vertexGroups[g]) tomados.push_back(m->vertexGroups[g]->nombre);
    if (arrastra)
        for (size_t b = 0; b < rig->bones.size(); b++)
            if (rig->bones[b].name != viejo) tomados.push_back(rig->bones[b].name);
    const std::string nuevo = W3dNombreUnicoEnValores(pedido, "Group", tomados, -1);
    if (nuevo == viejo) return viejo;
    // los HUESOS viven POR VALOR en rig->bones: se guardan como (armature, indice), NO como
    // std::string* (un bonenew posterior realoca el vector y el undo escribia en memoria
    // liberada, dejando el binding roto en silencio). Ver W3dRenameDest en Undo.h.
    std::vector<W3dRenameDest> aRenombrar;
    // el VERTEX GROUP tampoco va por puntero: Mesh::LiberarCapas lo destruye y MeshGeoUndo lo
    // rehace con new (cualquier op de geometria apilada encima), y el boton "-" lo borra ->
    // (malla, lista, indice), igual que los huesos. Ver W3dRenameDest en Undo.h.
    aRenombrar.push_back(W3dDestCapaMalla(m, W3dRenameDest::VGroup, idx));
    int nb = 0;
    if (rig)
        for (size_t b = 0; b < rig->bones.size(); b++)
            if (rig->bones[b].name == viejo) { aRenombrar.push_back(W3dDestHueso3D(rig, (int)b)); nb++; }
    ...
    if (nuevo != W3dNombreNormalizar(pedido, "Group"))
        W3dAvisoYaExiste(W3dNombreNormalizar(pedido, "Group"), nuevo);
    return nuevo;
}
...
// el swap NO se hace aca: lo hace UndoMoverCapaMalla, que ademas deja el paso de undo y
// REMAPEA los destinos de rename ya capturados (van por indice; ver Undo.h). Antes esto
// reordenaba a mano, sin undo, y dejaba apuntando a otro elemento a los renames del stack.
void MoverVertexGroupActivo(Mesh* m, int dir) {
    if (!m) return;
    UndoMoverCapaMalla(m, W3dRenameDest::VGroup, m->grupoActivo, m->grupoActivo + dir);
}

// UV GROUPS (pesos por CORNER del editor UV / armature 2D). MISMO patron que los vertex groups,
...
// RENAME de un UV group: uniquifica en la MALLA y arrastra el HUESO 2D homonimo
// (binding por nombre del rig 2D, ver Mesh::Armature2DAplicar), en UN paso de undo.
std::string UVGroupRenombrar(Mesh* m, int idx, const std::string& pedido) {
    if (!m || idx < 0 || idx >= (int)m->uvGroups.size() || !m->uvGroups[idx]) return std::string();
    const std::string viejo = m->uvGroups[idx]->nombre;
    // MISMA regla que VertexGroupRenombrar: si el rename ARRASTRA huesos 2D homonimos, el nombre
    // tiene que quedar libre tambien entre los huesos 2D (sino quedaban dos huesos con el mismo
    // nombre). Sin arrastre, ponerle a un UV group el nombre de un hueso 2D existente es legal.
    bool arrastra = false;
    for (size_t a = 0; a < m->armatures2d.size() && !arrastra; a++) {
        ...
        for (size_t i = 0; i < arm->huesos.size() && !arrastra; i++)
            if (arm->huesos[i].nombre == viejo) arrastra = true;
    }
    std::vector<std::string> tomados;
    for (size_t g = 0; g < m->uvGroups.size(); g++)
        if ((int)g != idx && m->uvGroups[g]) tomados.push_back(m->uvGroups[g]->nombre);
    if (arrastra)
        for (size_t a = 0; a < m->armatures2d.size(); a++) {
            Armature2D* arm = m->armatures2d[a]; if (!arm) continue;
            for (size_t i = 0; i < arm->huesos.size(); i++)
                if (arm->huesos[i].nombre != viejo) tomados.push_back(arm->huesos[i].nombre);
        }
    const std::string nuevo = W3dNombreUnicoEnValores(pedido, "UV Group", tomados, -1);
    if (nuevo == viejo) return viejo;
    // MISMO cuidado que en VertexGroupRenombrar: los huesos 2D viven POR VALOR en
    // arm->huesos -> (malla, armature, indice), no un std::string* que un bone2dnew realoca.
    std::vector<W3dRenameDest> aRenombrar;
    aRenombrar.push_back(W3dDestCapaMalla(m, W3dRenameDest::UVGroup, idx)); // idem: se libera y se recrea
    int nb = 0;
    for (size_t a = 0; a < m->armatures2d.size(); a++) {
        Armature2D* arm = m->armatures2d[a];
        if (!arm) continue;
        for (size_t i = 0; i < arm->huesos.size(); i++)
            if (arm->huesos[i].nombre == viejo) { aRenombrar.push_back(W3dDestHueso2D(m, arm, (int)i)); nb++; }
    }
    ...
    if (nuevo != W3dNombreNormalizar(pedido, "UV Group"))
        W3dAvisoYaExiste(W3dNombreNormalizar(pedido, "UV Group"), nuevo);
    return nuevo;
}
...
void MoverUVGroupActivo(Mesh* m, int dir) {   // idem MoverVertexGroupActivo (undo + remapeo)
    if (!m) return;
    UndoMoverCapaMalla(m, W3dRenameDest::UVGroup, m->uvGrupoActivo, m->uvGrupoActivo + dir);
}

// ============================================================================
//  ACARREO DE LAS CAPAS POR-CORNER POR LAS OPS DE TOPOLOGIA (las 4 de abajo).
//
//  LAS CAPAS 'Per-Vertex' VIAJAN COMO TODAS LAS DEMAS. Las cuatro funciones
//  saltaban las capas de color con porVertice=true, y eso NUNCA fue correcto: el
//  flag es de PRESENTACION (el toggle Per-Vertex/Per-Corner de la tarjeta
//  Vertices, Properties.cpp), el ALMACENAMIENTO es SIEMPRE por corner -aca y en
//  el archivo, donde por=vertice es una CLAVE del bloque COL, no otro layout-.
//  Saltearlas dejaba la capa con la cantidad de corners VIEJA; mientras las capas
//  de color no se guardaban nunca eso era invisible, pero ahora son persistentes
//  y el escritor tira toda capa que no mida lo que la topologia dice: un extrude
//  seguido de un guardado BORRABA la capa entera y para siempre.
// ============================================================================
...
void ReverseCapasDeCorner(Mesh* m, int L, int count) {
    ...
    for (size_t k=0;k<m->colorLayers.size();k++){
...
void CompactarCapas(Mesh* m, const std::vector<int>& survCorner) {
    for (size_t k=0;k<m->uvMaps.size();k++){ std::vector<GLfloat>& u=m->uvMaps[k]->uv; std::vector<GLfloat> nu; nu.reserve(survCorner.size()*2);
        ...
        u.swap(nu); }
    for (size_t k=0;k<m->colorLayers.size();k++){ std::vector<GLubyte>& c=m->colorLayers[k]->color; std::vector<GLubyte> nc; nc.reserve(survCorner.size()*4);
        for (size_t i=0;i<survCorner.size();i++){ int L=survCorner[i];
...
void ReconstruirCapasDesde(Mesh* m, const std::vector<CornerSrc>& src) {
    ...
    for (size_t k=0;k<m->colorLayers.size();k++){ const std::vector<GLubyte>& c=m->colorLayers[k]->color; std::vector<GLubyte> nc; nc.reserve(src.size()*4);
...
void Mesh::GenerarRender(bool recomputarNormales) {
    ...
    // nuevo -> viejo DIRECTO (para la vertex anim): cada vert nuevo sabe su vert VIEJO de origen.
    // OJO: NO invertir oldToNew (solo guarda el 1er destino; un vert que se SPLITEA en varios por
    // normal/uv perderia los splits -> esos verts quedaban estaticos y la malla se DESGARRABA).
    ...
    for (size_t f=0;f<faces3d.size();f++) { MeshFace mf; mf.mat = faces3d[f].mat; mf.smooth = faces3d[f].smooth; // conserva mesh part + shading por cara
        for (size_t c=0;c<faces3d[f].idx.size();c++) {
            ...
            if (it!=mapa.end()) gi=it->second;
                ...
                // al REP de la posicion, que NO distingue SPLITS de costura de uv (cada split
                // tiene SU canal uv animado). Se AFINA el origen por split: el vert viejo con
                // la MISMA posicion que el rep y el MISMO uv vivo que este corner -> receta
                // soloUV propia (a==b = copia). Sin match queda la receta del rep (fallback).
                int ovAnim = gv;
                if (tUV && !vtxAnimRecipes.empty() && gv >= vtxAnimRecipeBase){
                    int ri0 = gv - vtxAnimRecipeBase;
                    if (ri0 >= 0 && ri0 < (int)vtxAnimRecipes.size() && vtxAnimRecipes[ri0].soloUV){
                        int rep = vtxAnimRecipes[ri0].a;
                        if (rep >= 0 && rep < vtxAnimRecipeBase){
                            for (int o = 0; o < vtxAnimRecipeBase; ++o){
                                if (vertex[o*3]!=vertex[rep*3] || vertex[o*3+1]!=vertex[rep*3+1] ||
                                    vertex[o*3+2]!=vertex[rep*3+2]) continue;      // otra posicion
                                if (uv[o*2]!=u0 || uv[o*2+1]!=v0) continue;        // otro split de uv
                                if (o != rep){
                                    VtxRecipe rc(o, o, 0.0f); rc.soloUV = true;
                                    vtxAnimRecipes.push_back(rc);
                                    ovAnim = vtxAnimRecipeBase + (int)vtxAnimRecipes.size() - 1;
                                }
                                break;
                            }
                        }
    ...
    // hay que remapearlos como las capas de vertex anim. Se usa newToOld (nuevo->viejo, SPLIT-AWARE:
    // dos render-verts nuevos que salieron del mismo viejo copian los dos), NO oldToNew (que se
    // queda con el ultimo split). Render-vert nuevo sin origen valido (loose/corte) = peso 0. Los
    // vertex groups (por control-point) NO se tocan aca: se re-derivan solos.
    if ((int)newToOld.size()==nuevoN){
        for (size_t g=0; g<uvGroups.size(); g++){
            UVGroup* ug = uvGroups[g];
            if (!ug || ug->verts.empty()) continue;
            std::vector<float> wViejo((size_t)oldToNew.size(), 0.0f); // densificar los pesos viejos
            for (size_t k=0;k<ug->verts.size() && k<ug->pesos.size();k++){
                int rv = ug->verts[k];
                if (rv>=0 && rv<(int)wViejo.size()) wViejo[(size_t)rv] = ug->pesos[k];
            }
            ug->verts.clear(); ug->pesos.clear();
            for (int n=0;n<nuevoN;n++){
                int o = newToOld[n];
                if (o<0 || o>=(int)wViejo.size()) continue;
                float w = wViejo[(size_t)o];
                if (w>0.0f){ ug->verts.push_back(n); ug->pesos.push_back(w); }
            }
        }
    }
    // VERTEX ANIMS: la topologia cambio -> los keyframes (paralelos a vertex[]) quedaron
    // desalineados. Se remapea con newToOld (nuevo->viejo GPU): los verts EXISTENTES siguen
    // la animacion; los NUEVOS van por su receta (corte: lerp de todas las capas; duplicar/
    // extrude: soloUV, el canal UV copia el del origen y las posiciones quedan en la pose
    // actual) o, sin receta, quedan enteros en la pose actual.
    // Sin esto la evaluacion leia fuera de rango al editar una malla animada.
    // newToOld[gi] = vert VIEJO de origen de cada vert nuevo (armado DIRECTO arriba, con los
    // splits). Deberia medir nuevoN; por seguridad se valida el tamano.
    if (!animations.empty() && (int)newToOld.size()==nuevoN)
        RemapVertexAnims(this, newToOld.empty()?0:&newToOld[0], nuevoN, (int)oldToNew.size());
    // las recetas de corte se CONSUMEN una vez (este GenerarRender): limpiarlas para que la
    // proxima op (ej: extrude) NO las reuse (sus verts nuevos van estaticos, no interpolados).
    vtxAnimRecipes.clear(); vtxAnimRecipeBase = 0;
    ...
    // fresco antes de aplicar el stack, queda bien al instante.
    CalcularBordes(); // recomputa posRep/edges/centroGeom + invalida el edit (geometria nueva)
    // WEIGHT PAINT / SKINNING en mallas del EDITOR: vertCtrlPoint (render-vert -> control-point) solo lo
    // llenaba el importador FBX/glTF -> en una malla modelada aca quedaba VACIO y ni ConstruirColorPeso ni
    // SkinearMesh andaban. Se puebla IDENTIDAD-POR-POSICION (posRep, recien recalculado): todas las copias
    // por costura (splits de uv/normal) comparten control-point -> pintar un vert pinta sus copias.
    if (vertCtrlPoint.empty()) {
        vertCtrlPoint.resize(vertexSize);
        for (int i = 0; i < vertexSize; i++)
            vertCtrlPoint[i] = ((int)posRep.size() == vertexSize) ? posRep[i] : i;
    } else {
        // malla que YA trae mapeo (importada o ya poblado): NO se pisa; el remap de arriba (oldToNew) lo
        // acarreo, pero los verts NUEVOS de un corte (loop cut / extrude) quedaron en -1 y no se podrian
        // pintar. Se les da un control-point: el del REPRESENTANTE por posicion si un vert viejo ya esta
        // en ese lugar (comparte pesos), o uno FRESCO por posicion (peso 0). Los pesos viejos siguen
        // validos (mismos CP). Mismo espiritu de coherencia que RemapVertexAnims.
...
void Mesh::CalcularBordes(bool invalidarEdit, bool reagruparPosRep) {
    ...
    if (!vertex || vertexSize <= 0) {
        posRep.clear();
        aabbOk = false; // sin geometria no hay AABB (el Culling no lo usa para cortar)
        // sin geometria tambien hay que invalidar el edit: sino, tras borrar TODO
        // en Edit Mode, el cage viejo quedaba dibujado (overlay fantasma)
        if (invalidarEdit) InvalidarEdit();
        PodarMarcasSinArista(); // sin un solo punto: ninguna marca puede sobrevivir
        return;
    }
    const int nV = vertexSize;
    ...
    if (invalidarEdit) InvalidarEdit();
    if (faces3d.empty() && looseEdges.empty()) { // sin caras ni bordes sueltos
        PodarMarcasSinArista();                  // ... entonces no hay NINGUNA arista: fuera todas
        return;
    }
    ...
    bordesGeomVersion = skinGeomVersion; // bordesBuf refleja ESTA version de vertex[]

    // ------------------------------------------------------------------
    //  AQUI SE CIERRA LA CLASE "las marcas sobreviven a operaciones que ya
    //  no las sostienen". sharpEdges/seamEdges tienen clave POR POSICION y
    //  su verdad es "existe una arista entre estas dos posiciones". edges[]
    //  ES esa verdad y se acaba de recalcular: este es EL punto donde se
    //  valida, y toda operacion (borrar, aplicar un modificador, extruir,
    //  cortar, separar, join, importar) desemboca aca -por GenerarRender o
    //  directo-, asi que NO hay que acordarse de podar en cada una.
    //
    //  ORDEN: si hay un W3dMoverVerts abierto la malla esta a medio mover y
    //  PodarMarcasSinArista se saltea sola (un movimiento re-ancla al cerrar
    //  el guard; podar antes borraria la marca en vez de seguir al vertice).
    //
    //  POSE: si una VERTEX ANIM tiene la malla POSADA (vertex[] en la pose del
    //  cuadro y las claves en las de REPOSO) tampoco se decide nada, por la
    //  misma razon: ahi TODA marca parece huerfana. Ver Mesh::PosadaPorVertexAnim.
    //
    //  Los que MUEVEN posiciones sin ser un move puro (el extrude crece los
    //  arrays y desplaza los verts interiores) re-anclan ELLOS antes de llegar
    //  aca: ver ExtrudeReanclarMarcas, arriba en este mismo archivo.
    // ------------------------------------------------------------------
    PodarMarcasSinArista();
}
...
// OJO: es la MITAD de Mesh::SharpEdgeKey, que vive en el CORE (Mesh.cpp) porque el
// Core la usa (PodarMarcasSinArista) y tiene que linkear SIN el editor.
...
void Mesh::CornerNormalConSharp(){
    ...
    L = 0;
    // las CLAVES salen de PosicionesReposo (donde viven sharpEdges), NO de vertex[]: con la
    // malla POSADA por una vertex anim ninguna clave matcheaba y los bordes filosos se
    // ablandaban justo mientras se anima. Las NORMALES de arriba si van con vertex[]: esas
    // son la geometria real que hay que sombrear.
    const GLfloat* PK = PosicionesReposo();
    if (!PK) PK = vertex;
    for (size_t f = 0; f < faces3d.size(); f++) {
        const std::vector<int>& ring = faces3d[f].idx;
        int m = (int)ring.size();
        for (int k = 0; k < m; k++) {
            ...
            if (ia<0||ia>=vertexSize||ib<0||ib>=vertexSize) continue;
            std::string pkA = PosKey12(&PK[ia*3]);
            std::string pkB = PosKey12(&PK[ib*3]);
...
void Mesh::LiberarCapas(bool incluirGrupos) {
    ...
    // (incluirGrupos=false), sino se perdia el skinning y la malla colapsaba (bug del Ctrl+J sobre mallas skinneadas).
    if (incluirGrupos) {
...
void Mesh::PoblarCapas() {
    ...
    bool stale =
        ...
        // el flag porVertice NO exime del chequeo: el almacenamiento de esa capa
        // tambien es por corner (ver el bloque de ReverseCapasDeCorner), asi que una
        // capa Per-Vertex con la cantidad vieja esta tan stale como cualquier otra.
        (!colorLayers.empty() && colorActivo>=0 && colorActivo<(int)colorLayers.size() &&
         (int)colorLayers[colorActivo]->color.size() != nC*4);
    if (stale) LiberarCapas(); // la geometria cambio -> rehacer (FASE 2b: remapear las NO-activas)
    if (uvMaps.empty() && uv) {
        UVMap* mp = new UVMap(NombreLibreUVMap("UVMap", -1)); mp->uv.resize((size_t)nC*2);
        int L=0; for (size_t f=0;f<faces3d.size();f++) for (size_t c=0;c<faces3d[f].idx.size();c++){
    ...
    // NORMAL autoritativa por corner: si no esta o quedo de otro tamano (op que no la
...
void Mesh::ReagruparMeshParts() {
    ...
    skinGeomVersion++;
    W3dPVSInvalidar(this); // PVS: los tris cambiaron de lugar -> re-armar el index buffer del sector activo
}

// ---------------------------------------------------------------------------
//  mapa render-vert -> control-point si esta VACIO: identidad POR POSICION
//  (posRep) para que las copias por costura (splits de uv/normal) compartan
//  peso. GenerarRender hace lo mismo al reconstruir; esto cubre la malla RECIEN
//  creada (primitivas) que aun no paso por ahi.
//
//  Vive ACA y no en WeightPaint.cpp (de donde vino) porque la llama la CARGA de
//  una malla con esqueleto -- o sea tambien el runtime de un juego compilado, que
//  no linkea el pincel/popup/undo del modo Weight Paint. Es dato de la malla.
// ---------------------------------------------------------------------------
void WeightPaintAsegurarMapa(Mesh* m) {
    if (!m || m->vertexSize <= 0 || !m->vertCtrlPoint.empty()) return;
