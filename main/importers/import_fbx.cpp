...
struct Rd {
    ...
    bool avail(size_t n) const { return n <= (size_t)(end - p); } // sin overflow de puntero (p+n podia wrappear en 32 bits)
...
static void LeerProp(Rd& r, FProp& pr) {
    ...
    switch (pr.type) {
        ...
        case 'f': case 'd': case 'l': case 'i': case 'b': {
            ...
            if (enc == 1) { // zlib
                // cotas contra un archivo corrupto: clen dentro del buffer y una
                // expansion plausible (deflate no pasa de ~1032:1). Sin esto un
                // 'len' gigante del archivo pedia gigas de RAM (bad_alloc sin catch).
                if (!r.avail(clen) || rawBytes > (size_t)clen * 1032u + 64u) { r.p = r.end; break; }
                raw.resize(rawBytes ? rawBytes : 1); // nunca vacio -> &raw[0] valido (C++03: vector no tiene .data())
                if (w3dEngine::Inflate(r.p, (int)clen, &raw[0], (int)rawBytes)) data = &raw[0];
                r.p += clen;
            } else {        // raw
...
static Mesh* MallaDesdeGeometry(const FNode& geo, const std::string& nombre, const std::vector<Material*>& mats, Object* parent,
    ...
    mesh->SetNameObj(nombre); // nombre UNICO en la escena destino
    int a0 = 0, a1 = 0, a2 = 0;
    ...
    if (!mesh->normals && mesh->vertexSize > 0) { // sin normales -> smooth (como el OBJ)
        ...
        // CON normales del FBX: detectar smooth/flat para que al editar un vertice no se recalcule todo flat
...
static void AplicarTransformFBX(Object* o, double escala, bool rotar, bool escalar) {
    ...
    if (rotar) o->SetRotAxisAngle(Vector3(1.0f, 0.0f, 0.0f), -90.0f);   // la puerta deja rotEuler al dia (es lo que se guarda)
    else       o->ActualizarDisplayRot();                                 // sin rotar: solo refrescar los displays
}
...
static void ParsearMaterialesFBX(const FNode& root, const FNode& objs, const std::string& filepath, MatsFBX& out) {
    ...
    for (std::set<long long>::iterator it = matIds.begin(); it != matIds.end(); ++it) {
        ...
        // (el ctor de Material YA se registra en Materials: no duplicar)
        out.porId[id] = mat;
...
bool ImportFBX(const std::string& filepath) {
    ...
    if (matsInfo.porId.empty()) {
        ...
        for (size_t i = 0; i < texs.size() && !matFallback; i++) {
            ...
            if (!res.empty()) {
                ...
                // (el ctor de Material ya lo registro en Materials)
    ...
    if (esq.hay()) {
        ...
        arm->SetNameObj(ExtractBaseName(filepath) + "_rig"); // nombre UNICO
        arm->bones = esq.bones;
        ...
        // queda en AnimFPS (default 30): el banana dice 24 pero se ve bien a 30 (metadato del FBX incorrecto).
        ...
        // los dos. banana/otros Z-up coinciden FK<->TL -> no se tocan.
        {
            ...
            if (tlY > tlZ * 1.2f && fkZ > fkY * 1.2f){ // TL parado (Y) + FK-rest acostado (Z), con margen
                arm->SetRotAxisAngle(Vector3(1.0f, 0.0f, 0.0f), 90.0f);
