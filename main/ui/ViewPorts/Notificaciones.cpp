...
void Notificar(const std::string& msg, bool error) {
    ...
    // a leerlo. Ademas se pueden cerrar tocando el mensaje (NotificacionesClick). (exito 6s, error 10s).
...
void NotificacionesRender(int screenW, int screenH) {
    ...
    for (int i = (int)gNotifs.size() - 1; i >= 0; i--) { // viejo abajo -> nuevo arriba
        ...
        // hasta 3 lineas: un mensaje largo NUNCA se recorta a media palabra sin avisar
