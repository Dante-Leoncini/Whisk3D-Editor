...
#include "W3dPaletas.h"       // pestania "Pal": paleta efectiva + paletas del proyecto
#include "objects/Objects.h"  // Object::paleta (la seleccion del objeto)
...
// ============================================================================
//  EL PICKER NO PUEDE SOBREVIVIR A SU DUENO
//
//  El popup se queda con tres punteros ADENTRO del elemento que edita: 'target' (el float[4]
//  del color, que ESCRIBE en vivo), 'palRef' (el campo indice pal* del elemento) y 'palObj'
//  (el elemento). Ninguno es una copia: son memoria del objeto. Si ese objeto se borra, son
//  punteros muertos, y con la direccion reciclada por el objeto siguiente el picker pasa a
//  escribir el color / la paleta de OTRO elemento, en silencio (la familia de siempre).
//  Dos agujeros y dos tapas:
//   - CERRADO: Cerrar() no los limpiaba, asi que quedaban colgados hasta la proxima apertura
//     (Abrir los resetea). Ahora se sueltan al cerrar: el estado no sobrevive a la sesion de
//     edicion que lo creo.
//   - ABIERTO: si borran el objeto con el picker abierto no alcanza con limpiar al cerrar
//     (justamente, Cerrar() ESCRIBE en 'target' para capturar el paso de undo). Por eso se
//     engancha en el aviso de destruccion de Object (W3dDesvincularRegistrar, Objects.h):
//     el picker suelta todo y se cierra sin tocar la memoria del muerto.
// ============================================================================
static void ColorPickerDesvincular(Object* borrado){
    if (!colorPicker || !borrado) return;
    if (colorPicker->palObj != borrado) return;   // no es el elemento que estamos editando
    colorPicker->OlvidarDueno();
}
struct ColorPickerEnganchar { ColorPickerEnganchar(){ W3dDesvincularRegistrar(ColorPickerDesvincular); } };
static ColorPickerEnganchar g_colorPickerEnganche;

// suelta TODO lo que apunta adentro del elemento y cierra el popup si estaba abierto. No
// desreferencia nada (el dueno se esta destruyendo): ni captura el paso de undo del color,
...
ColorPicker::ColorPicker() : PopUpBase("Color") {
    ...
    tabs[1] = new Tab("HSV");
    tabs[2] = new Tab("Hex");
    tabs[3] = new Tab("Pal");   // la pestania PALETA (solo con contexto: SetPaleta)
    nTabs = 3;
    palRef = NULL; palObj = NULL; onPaletaCambio = NULL;
    palScroll = 0; palVis = 0; palRowH = 10;
    fila = 0;
...
int ColorPicker::Filas() const {
    if (pestania == 3)   // Paleta: [Propio + colores] + [Igual que el padre + paletas]
        return PalCantColores() + PalCantPaletas() + 2;
    return (pestania == 2) ? 1 : 4; // Hex es una sola fila
}

// ---------------- pestania PALETA ----------------

int ColorPicker::PalCantColores() const {
    std::vector<PaletaColor>* cs = palObj ? W3dColoresEfectivos(palObj) : NULL;
    return cs ? (int)cs->size() : 0;
}

int ColorPicker::PalCantPaletas() const {
    return (int)W3dPaletas().size();
}

// fila SELECCIONABLE k -> linea de display (las 2 cabeceras no se seleccionan):
// linea 0 = cabecera "Colores"; 1..1+nC = Propio + colores;
// linea 2+nC = cabecera "Paletas"; 3+nC.. = Igual que el padre + paletas
int ColorPicker::PalLineaDeFila(int k) const {
    int nC = PalCantColores();
    return (k <= nC) ? 1 + k : 2 + k;
}

void ColorPicker::PalAsegurarVisible() {
    if (pestania != 3 || fila < 0) return;
    int d = PalLineaDeFila(fila);
    int nC = PalCantColores();
    // al pararse en la PRIMERA fila de una seccion conviene ver su cabecera
    if (fila == 0) d = 0;
    else if (fila == nC + 1) d = d - 1;
    if (d < palScroll) palScroll = d;
    int dFila = PalLineaDeFila(fila);
    if (dFila >= palScroll + palVis) palScroll = dFila - palVis + 1;
}

// ejecuta la fila: 0 = color PROPIO; 1..nC = referenciar ese INDICE de la
// paleta efectiva; nC+1 = heredar del padre; nC+2.. = elegir esa PALETA para
// el objeto (el swap: sus herederos se re-pintan al instante)
void ColorPicker::PalAplicarFila(int k) {
    if (!palRef || !palObj) return;
    int nC = PalCantColores();
    ...
    if (k == 0) {
        ...
        *palRef = k - 1;
        ...
        int j = k - nC - 2;
        if (j >= 0 && j < (int)ps.size()) palObj->paleta = ps[j].nombre;
    }
    if (onPaletaCambio) onPaletaCambio();
    // cambiar la paleta puede cambiar la cantidad de colores efectivos
    if (fila >= Filas()) fila = Filas() > 0 ? Filas() - 1 : 0;
    Reflow();
}
...
// habilita la pestania "Pal": ref = el campo indice pal* del elemento (-1 =
// color propio), obj = el elemento (su paleta efectiva y su seleccion),
// onCambio = aviso al panel (re-bind + redraw). Llamar DESPUES de Abrir.
void ColorPicker::SetPaleta(int* ref, Object* obj, void (*onCambio)()) {
    palRef = ref; palObj = obj; onPaletaCambio = onCambio;
    nTabs = (ref && obj) ? 4 : 3;
    // si el campo YA referencia un indice, abrir directo mostrando la paleta
    if (nTabs == 4 && *ref >= 0) { pestania = 3; fila = 0; palScroll = 0; }
    Reflow();
}

// layout (pixeles del popup, escalados). El ALTO depende de la pestania: la
// lista de la pestania Paleta crece con los colores/paletas (con tope de
// pantalla + scroll); las otras conservan su geometria de siempre.
void ColorPicker::Reflow() {
    ...
    // pestanias RGB / HSV / Hex (+ Pal) (se funden con las filas de abajo)
    for (int i = 0; i < nTabs; i++) tabs[i]->Resize(100000);
    tabAlto = tabs[0]->height;
    tabsY = circY + circLado + gapGS * 2;
    filasY = tabsY + tabAlto; // sin gap: la pestania se funde

    int w = prevX + prevW + pad;
    int wTabs = pad * 2 + gapGS * (nTabs - 1);
    for (int i = 0; i < nTabs; i++) wTabs += tabs[i]->width;
    if (w < wTabs) w = wTabs;
    // los 3 valores juntos en una tarjeta (margen de 1px entre rellenos)
    grupoH = 3 * RenglonHeightGS + 4 * GlobalScale;
    if (pestania == 3) {
        // la LISTA: 2 cabeceras + filas seleccionables; si no entra en la
        // pantalla se recorta y scrollea (el foco de teclado la sigue)
        palRowH = RenglonHeightGS + GlobalScale * 2;
        int lineas = Filas() + 2;
        int fijo = filasY + gapGS + gapGS + (RenglonHeightGS + bordersGS) + pad + gapGS;
        int maxLineas = (MenuPantallaH - fijo) / palRowH;
        if (maxLineas < 3) maxLineas = 3;
        palVis = (lineas < maxLineas) ? lineas : maxLineas;
        int maxScroll = lineas - palVis; if (maxScroll < 0) maxScroll = 0;
        if (palScroll > maxScroll) palScroll = maxScroll;
        if (palScroll < 0) palScroll = 0;
        alphaY = 0;   // sin fila de alpha ni boton de unidad en esta pestania
        btnY = filasY + gapGS + palVis * palRowH + gapGS;
        btnOkY = btnY;
    } else {
...
void ColorPicker::Cerrar() {
    ...
    // SOLTAR lo que apunta adentro del elemento: 'target' (el float[4] del color), 'palRef' (su
    // campo indice) y 'palObj' (el elemento). Nada de eso puede sobrevivir al cierre: el objeto
...
// ---------------- UV del circulo cromatico (ver ColorPicker.h) ----------------
// arte de 32x32 en el (96,70) de font.png; font.png esta en el (0,0) del atlas
static const float kRuedaPx = 32.0f, kRuedaX = 96.0f, kRuedaY = 70.0f;
static float gRuedaU0 = kRuedaX / 128.0f,                 gRuedaV0 = kRuedaY / 128.0f;
static float gRuedaU1 = (kRuedaX + kRuedaPx) / 128.0f,    gRuedaV1 = (kRuedaY + kRuedaPx) / 128.0f;

void CalcColorWheelUV(int texW, int texH) {
    if (texW <= 0 || texH <= 0) return;
    gRuedaU0 = kRuedaX / (float)texW;  gRuedaU1 = (kRuedaX + kRuedaPx) / (float)texW;
    gRuedaV0 = kRuedaY / (float)texH;  gRuedaV1 = (kRuedaY + kRuedaPx) / (float)texH;
}

void ColorWheelUV(float* u0, float* v0, float* u1, float* v1) {
    if (u0) *u0 = gRuedaU0; if (v0) *v0 = gRuedaV0;
    if (u1) *u1 = gRuedaU1; if (v1) *v1 = gRuedaV1;
}
...
void ColorPicker::Render() {
    ...
    // circulo cromatico: arte de 32x32 en (96,70) del atlas de 128x128
    {
        ...
        float u0, v0, u1, v1;
        ColorWheelUV(&u0, &v0, &u1, &v1);   // contra el tamano REAL del atlas (ver ColorPicker.h)
        cuv[0] = u0; cuv[1] = v0;
        cuv[2] = u1; cuv[3] = v0;
        cuv[4] = u0; cuv[5] = v1;
        cuv[6] = u1; cuv[7] = v1;
    ...
    // pestanias RGB / HSV / Hex (+ Pal) + el toggle de unidad (0-255 / 0-100%)
    {
    ...
    // ---- pestania PALETA: colores de la paleta EFECTIVA del objeto (click =
    // el campo referencia ese INDICE; "Propio" = color propio) y las PALETAS
    // del proyecto (click = la paleta del objeto; "Igual que el padre" =
    // heredar). La fila actual se marca con un punto accent.
    if (pestania == 3) {
        int izq = borderGS + marginGS;
        int cajaW = popUpWindow->width - izq * 2;
        std::vector<Paleta>& ps = W3dPaletas();
        std::vector<PaletaColor>* cs = palObj ? W3dColoresEfectivos(palObj) : NULL;
        Paleta* efectiva = palObj ? W3dPaletaEfectiva(palObj) : NULL;
        int nC = cs ? (int)cs->size() : 0;
        int nP = (int)ps.size();
        int lineas = nC + nP + 4;   // 2 cabeceras + (1+nC) + (1+nP)
        int yFila = filasY + gapGS;
        int cb = RenglonHeightGS;   // lado del swatch
        for (int d = palScroll; d < lineas && d - palScroll < palVis; d++, yFila += palRowH) {
            // que es la linea d (ver PalLineaDeFila)
            bool cabecera = (d == 0 || d == 2 + nC);
            if (cabecera) {
                w3dEngine::PushMatrix();
                w3dEngine::Translatef((GLfloat)(izq + gapGS), (GLfloat)(yFila + GlobalScale), 0);
                w3dEngine::Color4f(grisUI[0], grisUI[1], grisUI[2], 1.0f);
                RenderBitmapText(d == 0 ? "Colores" : "Paletas", textAlign::left, cajaW);
                w3dEngine::PopMatrix();
                continue;
            }
            int k = (d <= 1 + nC) ? d - 1 : d - 2;   // fila seleccionable
            // la tarjeta de la fila (borde blanco = foco de teclado)
            filaCard->Resize(cajaW, palRowH - GlobalScale);
            w3dEngine::PushMatrix();
            w3dEngine::Translatef((GLfloat)izq, (GLfloat)yFila, 0);
            w3dEngine::Color4f(fondo[0], fondo[1], fondo[2], 1.0f);
            ...
            // marcador de SELECCION actual (punto accent a la izquierda)
            bool marcado = false;
            std::string etiqueta;
            const float* sw = NULL;   // swatch (solo filas de color)
            if (k == 0) { etiqueta = "Propio"; marcado = palRef && *palRef < 0; }
            else if (k <= nC) {
                etiqueta = (*cs)[k - 1].nombre;
                sw = (*cs)[k - 1].rgba;
                marcado = palRef && *palRef == k - 1;
            } else if (k == nC + 1) {
                etiqueta = "Igual que el padre";
                marcado = palObj && palObj->paleta.empty();
            } else {
                int j = k - nC - 2;
                etiqueta = ps[j].nombre;
                marcado = palObj && palObj->paleta == ps[j].nombre;
                // sin seleccion propia: senalar la HEREDADA (la efectiva)
                if (!marcado && palObj && palObj->paleta.empty() && efectiva == &ps[j])
                    etiqueta += " (heredada)";
            }
            int tx = gapGS * 2;
            if (marcado) {
                w3dEngine::Disable(w3dEngine::Texture2D);
                w3dEngine::Color4f(accent[0], accent[1], accent[2], 1.0f);
                rect->SetSize((GLshort)tx, (GLshort)((palRowH - GlobalScale * 3) / 2),
                              (GLshort)(3 * GlobalScale), (GLshort)(3 * GlobalScale));
                rect->RenderObject(false);
                w3dEngine::Enable(w3dEngine::Texture2D);
            }
            tx += 5 * GlobalScale;
            if (sw) {
                // el SWATCH del color (sobre un fondo negro por el alpha)
                w3dEngine::Disable(w3dEngine::Texture2D);
                w3dEngine::Color4f(0, 0, 0, 1.0f);
                rect->SetSize((GLshort)tx, (GLshort)GlobalScale, (GLshort)cb,
                              (GLshort)(palRowH - GlobalScale * 3));
                rect->RenderObject(false);
                w3dEngine::Color4f(sw[0], sw[1], sw[2], sw[3]);
                rect->SetSize((GLshort)(tx + GlobalScale), (GLshort)(GlobalScale * 2),
                              (GLshort)(cb - GlobalScale * 2),
                              (GLshort)(palRowH - GlobalScale * 5));
                rect->RenderObject(false);
                w3dEngine::Enable(w3dEngine::Texture2D);
                tx += cb + gapGS * 2;
            }
            w3dEngine::PushMatrix();
            w3dEngine::Translatef((GLfloat)tx, (GLfloat)GlobalScale, 0);
            if (fila == k) w3dEngine::Color4f(blanco[0], blanco[1], blanco[2], 1.0f);
            else w3dEngine::Color4f(grisUI[0], grisUI[1], grisUI[2], 1.0f);
            RenderBitmapText(etiqueta, textAlign::left, cajaW - tx - gapGS);
            w3dEngine::PopMatrix();
            w3dEngine::PopMatrix();
        }
        // hay mas filas arriba/abajo: indicadores chicos al borde derecho
        if (palScroll > 0 || palScroll + palVis < lineas) {
            w3dEngine::PushMatrix();
            w3dEngine::Translatef((GLfloat)(izq + cajaW - gapGS * 3), (GLfloat)(filasY + gapGS), 0);
            w3dEngine::Color4f(grisUI[0], grisUI[1], grisUI[2], 1.0f);
            if (palScroll > 0) RenderBitmapText("^", textAlign::left, gapGS * 4);
            w3dEngine::PopMatrix();
            if (palScroll + palVis < lineas) {
                w3dEngine::PushMatrix();
                w3dEngine::Translatef((GLfloat)(izq + cajaW - gapGS * 3),
                                      (GLfloat)(filasY + gapGS + (palVis - 1) * palRowH), 0);
                w3dEngine::Color4f(grisUI[0], grisUI[1], grisUI[2], 1.0f);
                RenderBitmapText("v", textAlign::left, gapGS * 4);
                w3dEngine::PopMatrix();
            }
        }
    }
...
void ColorPicker::AjustarFila(int delta) {
    if (!target || pestania >= 2) return; // hex: solo lectura; paleta: filas de accion
...
bool ColorPicker::Click(int mx, int my) {
    ...
    if (ly >= tabsY && ly < tabsY + tabAlto) {
        ...
        for (int i = 0; i < nTabs; i++) {
            if (lx >= tx && lx < tx + tabs[i]->width) {
                ...
                fila = 0;
                palScroll = 0;
                Reflow();   // el alto del popup depende de la pestania
                return true;
            }
            tx += tabs[i]->width + gapGS;
        }
    ...
    // pestania PALETA: click en una fila de la lista = ejecutarla (referenciar
    // el indice / volver a propio / elegir la paleta del objeto / heredar)
    if (pestania == 3) {
        int listaY = filasY + gapGS;
        if (ly >= listaY && ly < listaY + palVis * palRowH) {
            int d = palScroll + (ly - listaY) / palRowH;
            int nC = PalCantColores();
            int k = -1;   // las cabeceras (0 y 2+nC) no son seleccionables
            if (d >= 1 && d <= 1 + nC) k = d - 1;
            else if (d >= 3 + nC) k = d - 2;
            if (k >= 0 && k < Filas()) {
                fila = k;
                PalAplicarFila(k);
            }
...
// soltar en el valor exacto) y acelera DE A POCO (tope 8). Antes saltaba a 30 -> era demasiado rapido.
...
bool ColorPicker::Tecla(int tecla) {
    ...
    if (editCirculo) {
        float sat = 4.0f / 255.0f;              // SATURACION: paso fijo (la velocidad asi esta bien)
        switch (tecla) {
    ...
    switch (tecla) {
        case LayoutKey::Up:
            ...
            PalAsegurarVisible();                   // pestania Paleta: la lista sigue al foco
            return true;
        case LayoutKey::Down:
            holdDir = holdCount = 0;
        ...
        case LayoutKey::Right: {
            ...
            if (fila == -3) {                       // CIRCULO: der -> barra VALUE (esta a su derecha); el circulo se edita con OK
                ...
                holdDir = holdCount = 0;
                if (dir < 0) fila = -3;
            } else if (fila == -1) {                // PESTANIAS: cambia RGB / HSV / Hex (/ Pal)
                holdDir = holdCount = 0;
                pestania += dir; if (pestania < 0) pestania = nTabs - 1; if (pestania >= nTabs) pestania = 0;
                palScroll = 0;
                Reflow();                           // el alto del popup depende de la pestania
        ...
        case LayoutKey::Enter:
            if (fila == -3) {                       // CIRCULO: OK = entrar a editar (flechas mueven H/S)
                ...
                Cerrar();
            } else if (pestania == 3 && fila >= 0 && fila < filas) {
                PalAplicarFila(fila);               // pestania Paleta: ejecutar la fila (no cierra)
