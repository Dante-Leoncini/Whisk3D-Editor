...
Object* NewMesh(MeshType type, Object* parent, bool query){
    ...
    if (type == MeshType::plane || type == MeshType::cube ||
        ...
        // SetNameObj (no "name =") -> nombre UNICO: tres "add cube" dan Cubo/Cubo.001/Cubo.002
        mesh->SetNameObj((type == MeshType::cube)     ? T("Cube")
...
void Mesh::Regenerar(){
    DesinstanciarDatos(W3DMD_TODO, false);  // COW: la primitiva se regenera de cero (MallaDatos.h)
    Material* mat = materialsGroup.empty() ? MaterialDefecto : materialsGroup[0].material;
    // ESTADO DE POSE: esto REBUILDEA la geometria desde los parametros de la primitiva, o sea
    // que lo que queda en vertex[] es EL MODELO, no la pose de ningun cuadro. El estado viaja
    // con las posiciones tambien por esta puerta (ver Mesh::posadaPorAnim).
    posadaPorAnim = false;
